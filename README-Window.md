# README-Window —— 上游 `dotnet/wpf` 的原始 README（**逐字保留**）

> 本文件 = 上游 Windows 版 WPF 的 README **原文**（取自本仓 vendored 快照 `upstream/wpf/README.md`，MIT，见 `upstream/wpf/LICENSE.TXT`）。
> Linux 移植版的说明在 **[`README.md`](README.md)**（本仓主 README）；工程规范见 [`docs/PORT-SPEC.md`](docs/PORT-SPEC.md)，并行路线见 [`docs/ROUTES.md`](docs/ROUTES.md)。
> 保留原文是为了：① 上游身份与许可信息可追溯；② 与上游同步时能一眼看出差异。
