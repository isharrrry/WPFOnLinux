...
> 🧭 **路径过渡（2026-09-26 · P0 迁移，追加注；上方原文一字未改）**：**开发树已从「无 git 的 `$R`」换成 clone 本身** —— `~/netTest/GitProj/WPFOnLinux`（`N`）现在是**唯一工作树**（带 git），`$R` 的旧路径 `/home/links-dev/netTest/wpf-linux-20260906/wpf-linux` **仍在、且解析到活树**：它是**指向 `N` 的符号链接**（`#76` 原树整目录改名留在同级 `.p0shadow-wpf-linux` 作冻结证据）。⇒ §0 前提 1 那句「`$R` 本身不是 git 仓，也不要把它变成 git 仓」的**现状读法**改为：**该路径现在经由符号链接解析到带 git 的 `N`**；本件 §1／§2／§4 的历史命令**原文保留**（它们记录的是首发当时的做法）。回退 = `rm <旧路径> && mv .p0shadow-wpf-linux wpf-linux`。⚠️ **机器实测的坑**：裸 `find "<旧路径>"`（无尾斜杠）**不下钻**（GNU find 对符号链接起点不跟随）⇒ **一律以 `N` 为 cwd**，或在旧路径下用 `cd -P`／`find "<旧路径>/"`；仓内只有 `build/MilBridge/tools/boundary-decl-check.sh` 用裸根 `find`（实测：旧路径 cwd ⇒ `BOUNDARY_DECL=FAIL records=2 pass=1 fail=1`／cwd=`N` ⇒ `PASS`／`cd -P` ⇒ `PASS`／`--repo <physical>` ⇒ `PASS`）。细节见 `build/MilBridge/P0-migrate-report.md` §7。
