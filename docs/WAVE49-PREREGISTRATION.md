...
#### §13.4-⑦ 【方法论】**"验收必须有 WM"在这一族上正好反过来**（车道 W85A，2026-09-22）

现场（W85A 中期读数，判据先写死 `$HOME/w85a/criteria.md` `95d23c292007a5e9`）：

| 条件 | 读数 |
|---|---|
| 旧树＋**修前** `libwpfwin32.so`＋**无 WM** | **2/2 崩**：`rc=134`＋6.48/6.59 MB、几何 `112,84 800x600`、末击 `tab3`（= W63A `C101` 逐字段复刻） |
| **现树（只换树、shim 不变）**＋**无 WM** | **2/2 同样崩** ⇒ **崩由"修前 `.so` 单独"带来**（不是托管件/桥） |
| 同上两格但**有 WM** | **0/2 全活** —— 因为**点击从第 2 击起全被吞**（`AE=0`；`PC-C` 只有 **1/8** 击落地） |

⇒ **有 WM 时点击不落地 ⇒ 回声环根本不会被走到 ⇒ 只跑"有 WM"的装置永远测不到 `D-G66`**。
⚠️ 而本工程自 `D-G64` 起的默认口径是"**验收必须有 WM**"⇒ 在这一族上**正好反演**。**结论：两条腿都要（有 WM / 无 WM），并各自写明** —— 否则会把"装置够不到"读成"没复现"（`NOINFO`）。

**顺带更正（如实）**：W77A 的 `§5.1` 把 `C101` 的条件写成"**有 WM** 且有点击" ⇒ 它拿"有 WM"去复刻"无 WM"的阳性对照，得 0/2，遂判"这套装置不能归因"。**W63A 自己的 `run/C101/meta.txt` 第一行是 `DISP=:196`，而 WM 腿是 `:198`** ⇒ `C101` 是**无 WM** 趟 ⇒ **W77A 那句"不能归因"被推翻**（W85A 用只换 `.so` 的两臂已拿到 `134` 族的真两极化）。

**W85A 已拿到的两极化（`134` 响亮族，**不是**用户那个 `139`＋0 字节）**：无 WM 臂 —— 修后 `3e4390c9ec07f621` **9 击全落地**（`tab3 AE=182269` 真换页）⇒ 活满窗口 `rc=124`；修前 `abf6879c027c5e73` **第 8 击 `tab3` 整进程死** ⇒ `rc=134`＋6,449,034 B；栈顶闭合在 `HwndKeyboardInputProvider.FilterMessage ← OnSetFocus ← ReportInput ← … ← SetFocus`（`SetFocus` 14,676 行／`FilterMessage` 5,872 行）= **已登记的 `D-G66` 回声环**。⇒ **判决点 = 两个 `.so` 的改动集**（现场 `nm -D`：修前少 **14 个导出**、反向 0，全是 EWMH/窗口状态那一族）。

#### §14 【更正】`#49` §4 的"B3 接线 ⇒ `inputs_fp` 必变"**不成立`（车道 W84A 机械复核，2026-09-22）

- 复核方法（可复算）：机械抽出 `close-wave.sh` 的 `fp_inputs()` 覆盖面 = **144 件**，再拿本件新增/改动的件逐一对名 ⇒ **命中 0**（`verify-all.sh` 与两个 `R-GATE` 新件**都不在覆盖面内**）；09:10 之后唯一被改的**覆盖面成员**是**另一条车道**的 `src/WpfGfx.Linux.Native/src/win32_pts.c`。
- ⇒ **更正**：`#49` §4 那句预判作废；**是否把两件新判据件加进 `fp_inputs()`** 由主控裁定 —— **裁定：加，但在 `#50` 收尾链同趟加**（理由：`close-wave.sh` 自身在覆盖面内，波中盲改会让 `[4/6]` 报 `IN_FP_0 != IN_FP_1` 并 `exit 5`；收尾链在 `[0/6]` 之后才采 `IN_FP_0`，同趟改才安全），并写进 `repin --why`。
