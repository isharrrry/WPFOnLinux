...
> **一句话**：本仓的 `upstream/wpf/` 是 **dotnet/wpf（MIT）的一份被裁剪的快照**，由**非 git 手段**搬入（全树 6414 件落在 **2026-08-31 09:35** 同一秒级窗口内），**精确 commit 无法从树里定出**；本文给出 ①**可复算的完整性指纹**（改树即变）、②身份与世代的 in-tree 自证、③**裁剪面的成对证据**、④第三方可执行的"把 commit 钉死"配方。
>
...
| **精确 commit** | ✗ 无 `.git`、无 `Version.Details.xml`、树内无任何 commit 串 | **未定**（§6 只给窗口，§7 给配方） |
...
1. **精确 commit**：未定。树里没有 `.git`、没有 `Version.Details.xml`、没有任何 commit 串（全树 40-hex 只 1 处，且是源码内容）。
