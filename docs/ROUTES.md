...
│   │   └─ ⏪ **dated 口径说明（2026-09-24，主控）**：本行 ✅ 与 §12 区 `:148` 的 🟡 **不矛盾、是两种口径** —— ✅ 指"**零墨修法随 `#49` 落地生效**"（世代位 `hbtextline 921ba9c65e9fb3be` 在位、像素成对 `1/0.00% → 90/9.03%`）；🟡 指"**反极性腿未跑**"（拆掉修法 ⇒ 四区应回到 `1/8/19/69`）。⇒ **该任务只剩一件事：跑那条反极性腿**；跑完两行可同时标 ✅。
...
│   ├─ TASK-0723 [Next] 🔴 **`D-G124`：定位启动期 `EntryPointNotFoundException`（`SHAppBarMessage`）的机制** —— 新件代每次启动抛并自报，**两臂只差 `win32shim` 一件**可复现；**两件都没有 `SHAppBar*` 导出、也无 `shell32` 字样** ⇒ 差异不在符号表 ⇒ 机制 `NOINFO`；**两极化**：换回旧 shim ⇒ 该行**必须消失**；判定"修掉 or 按 `D-G76` 声明为已知降级"；排 `#68`
├─ 08xx R8 上游化与发布
