...
│   ├─ TASK-0302 [MVP] 🔴 PTS / 原生 LineServices（**可操作缺口 91 条／实现口径 97 条**，`D-G70`；旧数 111 已证 `TOOL-UNSOUND` ⇒ `TASK-0720`）
...
│   ├─ TASK-0720 [Next] 🔴 **`D-G70` 的在册数纠正 ＋ `Nl*` 有意降级声明** —— 「**111 条 `Fs*`/`Lo*` 缺口**」**复现不出来**：工具口径 **103** − **11 条 `#if NEVER`** − **1 条工具误报** ⇒ **可操作 91**（55 `Fs*`＋22 `Lo*`＋6 `Nl*`＋4 文本分析＋4 `*Wrapper`）｜**实现口径 97**（+6 条 `TASK-0304` 已导出但恒返回 `-10000`）——与 `docs/unimplemented.md` 的 `97（93+4）`、`W78A:22` 的 `109` **两个分解逐字闭合**；**按 111 估工作量虚高 22%**，且它在 **9 个文件**里被复述（含 C 源码注释）⇒ **逐处按内容锚更正**；并纠正 `W78A`：`#if NEVER` 死声明实为 **11 条不是 9**、其「真会炸 99 条」应为 **97**。**`Nl*` 6 条按 `D-G76` 体例在册声明为「有意降级」**（上游 `NaturalLanguageHyphenator.cs` 的 ctor `catch (EntryPointNotFoundException)` ＋ `SpellerInteropBase.cs` 同形吞掉；**不做 stub** —— `NlCreateHyphenator` 返回 `IntPtr` 句柄，返回 `-10000` = **假句柄** ⇒ 守卫当场失效）＋**一条牙**：断言 `IsHyphenationEnabled=True` 与 `=False` **行为等价**（真实现后**必红**）。⚠️ UI 入口在**仓外**（`hc` 第 24 页 `FlowDocumentDemo.xaml:9` 明写 `IsHyphenationEnabled="True"`／`IsOptimalParagraphEnabled="True"`）⇒ **第 24 页必走 `PtsCache` 的 optimal 分支**，`LoCreateContext` 才是真正下一跳。排 `#66`
...
- `TASK-0302` 🔴 PTS / LineServices（**可操作 91 条／实现口径 97 条**；在册 111 已证 `TOOL-UNSOUND`）
...
- `TASK-0111` [Next] 🟡 **判 `NOINFO` → `N1` 撤销/暂缓（**不撤号**，`ROUTES.md:720` 口径）**：`W115A` 报告现场判「**必要性 `NOINFO`（判不了）＋ 充分性被证伪**」（`none 0/200`／`hints 0/200`／`hints+geom 200/200`，`Fisher=1`）⇒ **分支 A 的判词不存在 ⇒ 落地处置被禁**；判据＋预备 diff 已备档 `~/w144a/`（`criteria.md 9f7af3f9a18cc869`／`prep.md 75bcc0857afe6ab2`） **`_MOTIF_WM_HINTS` 那一跳的落地**（`D-G98` 的**首选嫌疑**；来源 = `W114A` §8 第 1 条 ＋ §8b 的 `N1`）：
