...
- `TASK-0211` [Next] 🔴 **关掉落地修法里的两处窄 `TOCTOU`（"锁内取指针、锁外使用"）**（号现场核：`TASK-0201`…`TASK-0210` 已用、**`TASK-0211` 全仓 `0` 命中**；**不新缺陷号**，并入 `D-G109` 的 🔁🔁🔁 追加段）：
...
- 🆕 **新任务 `TASK-0110` [Next] 🟡 **前提被推翻 ＋ 改题（**两牙已落、接线待 `#59`**）**（`#59`：改题为「frame/client **时序判据** ＋ 桥侧几何重发的**确定性回归牙**」）："WM 不把退出最大化的几何收回去"**不成立** —— 台账 70 腿里红腿 **27/32 的 frame 回过基准**、**41–86 ms 后被桥顶回**；旧结论是**欠采样伪影**（记为 **`D-G112`**）。两块牙已落 `$R`：`geom-revert-beat-check.sh 9412ec0149111348`（12/12 自测 ＋ 真装置 4/4）／`geom-resend-regression-check.sh cd375326b62f7982`（8/8）**：**几何还原残留** —— 查"**WM 为什么不把退出最大化的几何收回去**"；判据**必须先写**且**必须能抓 frame/client 时序分离**（现行 `AFTER_R2`／`AFTER_R2_SETTLED` **两拍不够** ⇒ 要加中间拍）；反极性 = 修法拆掉后回到**同形**；取不到分层读数 ⇒ `NOINFO`。
...
- 🆕 **新任务 `TASK-0111` [Next] 🟡 **判 `NOINFO` → `N1` 撤销/暂缓（**不撤号**，`ROUTES.md:720` 口径）**：`W115A` 报告现场判「**必要性 `NOINFO`（判不了）＋ 充分性被证伪**」（`none 0/200`／`hints 0/200`／`hints+geom 200/200`，`Fisher=1`）⇒ **分支 A 的判词不存在 ⇒ 落地处置被禁**；判据＋预备 diff 已备档 `~/w144a/`（`criteria.md 9f7af3f9a18cc869`／`prep.md 75bcc0857afe6ab2`）**：**`_MOTIF_WM_HINTS` 那一跳的落地** —— **先决条件逐字 = 待 W115A**（纯 X 高功效探针、每臂 **≥200 趟**，钉"这次属性重写是不是**被打回**的必要条件"；本件写成时 `W115A` 报告**不存在**）；**分支 A（必要性成立）= "值没变就不写该属性" ＋ 反极性 ＋ 零回归 ＋ ≥40 腿/臂成对**；**分支 B（不成立）= 撤号或改写为"剩余候选"**。
...
- 🔢 **`TASK-0201` 上界读数更新**已按"**追加不改**"落在 **§14 清单的 `TASK-0201` 行下**（正下方一行；**原行一字未动**；`wc -l` 复核见 `build/MilBridge/WC01-report.md` §③）。
- 🧾 **补推两份证据报告**（此前只在 `$R` ⇒ 远端没有证据本体）：`build/MilBridge/W128A-report.md`（FULL sha256 `789d01e5f8959b0b70ffda770e8c02dac653661e48af5510f7e5a5d6ca485878`／352 行／37,548 B）＋ `build/MilBridge/W118A-report.md`（`head -n -2` sha16 **`a22eb8a7881e01ba`**／431 行／43,052 B）—— **推前占位符门**：`grep -n -F -e '<<<' -e '@@'` 逐件 **0 命中**（python 独立复算 `<<<` = 0 ∧ `@@` = 0，两法一致）⇒ **两件都推**（无一件因占位符被拦）。
- 🚚 **推送（本批一笔、纯文档；逐径 `git add`、绝不许 `-A`／`--force`）**：件清单、前后 head 与 `BYTECHECK` 逐字见 `build/MilBridge/WC01-report.md` §②。
...
- 🔴 **`D-G98` 的「一格定罪」终局 = 判词 `K3`（两个候选机制在应用侧都被推翻）**：出处 `build/MilBridge/W124A-report.md`（**FULL sha256 `b38a054b71f64ecc…`／去行口径 `head -n -2 … | sha16` = `df292e7faf2e0239`／339 行**，本件现场现算）＋ 判据 `~/w124a/criteria.md`（**封存值（追加 §7.1 之前）= `57a624cae1d217af`** —— ⚠️ 本件**截断复算未复现** ⇒ 该格记 `NOINFO`、按报告 §1 ＋ `~/w124a/STATUS.md:11,18` 两处留档**引用**；追加后现算 **FULL `c6f2055675fb9aaf`**）。要点（逐字段落在册内 `D-G98` 新追加段）：**提示全程是常量**（红腿逐字 `present=1 flags=0x10 PMinSize=1x1`、**`PMaxSize` 从未声明**；**主臂 60 腿 209 个 `SEG` 行去重后只有 1 个值**）｜**27/27 红腿几何序列 `800x600 → 1280x1024 → 800x600 → 1280x1024`**（先真还原、0.1 s 内被顶回）＋ `_NET_WM_STATE` **全程 `[FOCUSED]`**｜**阳性对照 `5/5` 有牙**（`PMinSize` 单独 `2/2` ＋ `PMinSize=PMaxSize` `3/3`）但**形态不同**（注入臂 `state` 仍是最大化）｜**分母 `60`、红 `27`、作废 1 趟**（`W124A-M2R2a-05` 宿主重启截断）。**本件 WC02 独立复算**（口径 = 读 `~/w124a/logs/W124A-M*/xhints.log` 的 `SEG` 行 ＋ 各腿 `probe.txt` 的 `RESULT` 行，**不经报告汇总表**）：**腿数 60（33 ＋ 27）｜`SEG` = 209｜`uniq(hints)` = 1｜`m_ok=1` = 60/60｜`r_ok2=0` = 27（`M1:R2` 24 ／ `M2:R2` 3）｜红腿四跳序列 27/27、`MAXIMIZED` 每条只出现 1 次、末态全 `[FOCUSED]`（0 例外）** ⇒ **与报告逐个字段相同**。⚠️ **取代声明**：§15m 那句「**首选成因 = 应用那份过期的尺寸约束挡住还原**」**在应用侧已被推翻、自此作废**（那份"过期约束"**从来不存在**）；该句**原话一字未动**，本行只声明取代关系。⚠️ **射程**：本件**只排除提示族**；"那 0.1 s 内**是谁**发的几何申请"**仍 `NOINFO`**（属 `N3` 写者归因）。⚠️ **臂间红率差 10 倍、判词一字不变**（`M2:R2` `3/33 = 9.1%` vs `M1:R2` `24/27 = 88.9%`）⇒ 差在**触发路径**、不在提示值。
- 🔁 **`TASK-0111` 要件改写（主控裁定 · dated）**：**① `N1` 撤销/暂缓**（依据 = 提示族已被 `K3` 排除 ⇒ 先落 `N1` 只会拿到"**红率不变**"的空结果；§15k／§15m 的旧理由"撤回不了已发布的过期上限/下限"**随之失效**）；**② 下一步 = `N3` 写者归因**，专抓"**还原后 0.1 s 内那一次几何申请**"，**靶子用 `M1:R2` 臂（88.9% 红）**；**状态位仍 🔴**。两条口径句（**(a)** 两臂 `hints` 逐字相同而红率差 10 倍 ⇒ **差在触发路径**；**(b)** **"同一装置换个臂就换红率"本身必须先写进预登记**）已逐字落在 `TASK-0111` 行下。
- 🟡 **`TASK-0110` 追加一行**：真因**部分排除**（提示族已排除；**应用侧几何申请已确证存在但未归因**）⇒ **状态位仍 🟡**（**不许**转 ✅）。
- 🧷 **仪器缺陷一条 → **并入 `D-G107`（不新号）** ＋ 交叉引用 `D-G64`**：X 观测器 `~/w124a/bin/xhints` 第一版**只遍历 `root` 的直接子窗** ⇒ 找不到被 **reparent** 的客户窗（目标是**孙代**）；**成对读数** = 修前件 `3e747a47869f00d1` ⇒ **`rc=5 reason=no-window`**（而 `xdotool` 独立确认窗 `10485764` 在场）vs 修后件 ⇒ **`WID=0xa00004`、`rc=0`**。**理由（先写后判）**：它是 `D-G107` 要件②「**取值时射程必须按该对象的真实结构定义**」的**第三个实例**（坏在"**遍历面**"一侧）；`D-G104` 被"**大声失败**（`rc=5` 非零 ＋ 自带 `reason=no-window`）"这一点**机械排除**（那条的形态是"**不报警**"）；**同一结构陷阱的产品侧先例 = `D-G64`**（`WindowFromPoint` 只返直接子窗 ⇒ 点击全被吞；修法逐字 = "**逐层下沉**"）⇒ 不满足"新号"条件。
- 🧾 **自伤照实记（不许缩小）**：**①** `D-G103` 追加**实例清点**（在册实例现为**四个**：`L14`／W124A／W126A／W127A）并**划清一条线** —— **W124A 那一例是 `~/w128a` 之外的另一个实例**（命中 **W118A 的 `gdb --args dotnet HandyControlDemo.dll` 装置**、代价记在 **`~/w118a`**：`G20301` 剔出分母 `41→40`；**与 `~/w128a` 的 `D-G106`／`D-G107`（实例 `W071`／`W077`）是两件不同的事** ⇒ 不许并条、不许把损失记错批次）；**"无法自证杀了几个进程"逐字保留**；**②** `D-G101` 追加**同族第三种机制**（`sed` 生成脚本**漏换 `W=$HOME/w105a`** ⇒ 4 件写进**禁改车道** `~/w105a/`；处置 = 按 PID 收进程、四件**移出留痕**到 `~/w124a/misfiled-w105a/`、复核残留 **0**、起 X 脚本**改手写**）＋ 口径句；**③** `D-G102` 追加**同族另一形态**（编排器**顺序错**：先起观测器等窗、后起腿 ⇒ 等一个**永远没人建的窗**（34 s 超时）⇒ **腿根本没跑而输出看着正常**；另一处 = 观测器输出放在腿会 `rm -rf` 的路径下 ⇒ **证据被删**；修法 = 腿后台先起 → 观测器等窗 → 腿前台等收 ＋ 目录分离 ＋ `FAILDIAG.txt` 留档）。
- 🔢 **`DEFREG` 两条机读行（现场跑两遍、逐字相同、`rc=0/0`，输出 `cmp` IDENTICAL）**：`DEFREG=PASS declared=145 route_ids=145`｜`DEFREG_DECLDRIFT=0`（本批**不开新号** ⇒ 与 §15p／§15q 的 `145` 相同；`--emit` 重生成后 **`ID` 行逐一 diff = 0 行** ⇒ **零幻影声明**）。⚠️ `--emit` 现场用**临时件 ＋ `rename`** 落盘（照 `D-G101` 教训：**不许** `--emit > 件` 原地截断），件 `links=1`、inode `4854448 → 4854450`。⚠️ **`DECL-ANCHORS` 里的 `KRJ=f108775906eac9aa` 仍是 `$R` 的现场值**（`#53` 在办件，本件**不许碰**）⇒ **远端**跑同一牙时该**非门禁诊断行**仍会读成 `DECLDRIFT=1`（唯一漂移项 `KRJ`），待 `#53` 推该件后自愈（与 §15q 同一条口径）。
- 🚚 **推送（本批一笔、纯文档；逐径 `git add`、绝不许 `-A`／`--force`）**：件清单、前后 head 与 `BYTECHECK` 逐字见 `build/MilBridge/WC02-report.md` §⑤。
...
- ✅ **`#53` 已冻结并推送**（三句都要有）：① **冻结** = `samples/WpfTextDemo/ACCEPTANCE-BASELINE.md` 整份 **`a2e49b786d0a1b02`**（**737,918 B**），`docs/CURRENT-STATE.md:9` = `gen=#53 sha16=a2e49b786d0a1b02`，**四颗牙全 PASS**（`BASELINESHA`／`BASELINEGEN decl_gen=#53 file_newest_gen=#53`／`BASELINE_BYTES=737918`／`BASELINEDUP n=0`，`rc=0`）——**主控现场重算，逐位相同**；② **冻后 `verify-all` ×2 = 两趟都 `步骤通过 27 ❌ 失败 0`／`结论：✅ 全部通过`／`用例通过 875 跳过 2`**（判词层机读行 **15/19 逐字相同**，4 处不同**全是运行期读数**：`mem_mb`／带时间戳目录名）；③ **推送** = 远端 `origin/feat-Linux` head **`d9a7e04fd8c431469a733821cd434720f553cb97`**，`ls-remote --symref origin HEAD` 仍 `ref: refs/heads/feat-Linux`（**主控现场核**）；远端 `docs/CURRENT-STATE.md` blob sha == 磁盘 **`81b35c059717b7a8`**（byte 一致）。
- 🔁 **`#53` 世代位移 = 只有两位**（**主控逐位复核**）：`win32shim 8392fc09564779a1 → … → bd037229be8db4f6 → **a6365183fa6d26b9**`（＝ `TASK-0109` 的产品位移）＋ **`pf 358136b0c806ee88 → 4973bcb28e331cf0`**（**同尺寸 6,123,520 B** ⇒ `D-G92` **第四次**再证：整波重建必换身份字节，**不许**当漂移/回归判据）；其余七位**逐位未变**（含 `hbtextline 921ba9c65e9fb3be`、`dwf ce3469f49efcbcfa` **Release 口径**）。native `build-shim.sh --all` **逐字节复现** `a6365183fa6d26b9`、**导出仍 547**。
- 📌 **`inputs_fp` = `5ac1e5349c7d1dec56fd7d8fe5dd8e9c1cb7c3b4052859f887e45c539981904f`**（覆盖面仍 **149 件**）；位移**唯一归因 = 重钉 `known-red.json`**（`d4e0080df6ec497c → f108775906eac9aa`）；W131A 那一笔的位移（三件 native 源）已包含在其前一值 `f0e2b3e8…` 里（**主控用真函数复算过**）。
- ✅ **门禁 ×2 = 12/12 PASS**（两趟各 `default 3/3`＋`env 3/3`，两本 rows 的 `config=` 逐字相同）｜**冻前 `verify-all` = `步骤通过 26 ❌ 失败 1`，唯一红 = `COLUMN-FLOOR`（声明类）**、`用例通过 875`（＝ 上代同值）、`[0]` 段 **`X-REUSE=reused :97`**（几何 1280x1024 相符 ⇒ `D-G105` 的守卫按设计工作）。
- ✅ **五臂**：**只 `tline` 应变**（`56bea723…` → `44c21d648f79126c`），**判词与 `#52` 逐字相同**（22/2、rc=1、rc=0、rc=0、10/0）。**重钉**：`FAIL n=2` → `APPLIED` → **`REPIN_GENERATION=PASS`**；`entries[*].caliber 改动字段数 = 0`。
- 🚚 **推送 3 笔、全部逐径 `git add`（无 `-A`）**：`aaa5bb5`（13 件）→ `067d719`（2 件）→ **`d9a7e04`（最终 head，1 件）**；每笔 `porcelain` 行数 == 暂存件数 == `cp -p` 的件数（**无夹带**），每笔 `BYTECHECK mismatch=0 nobody=0`；**6 件别的车道的报告未推**（不在克隆里且 W124A 当时仍在跑）。
- ✅ **app-local**：`STALE=0 DIVERGENT=0`；**门禁真正加载的那份** `libwpfwin32.so` = **`a6365183fa6d26b9`**（`APP_ART` 行）。**哨兵两处**：`WIN32SHIM=a6365183fa6d26b9`／`WAVE=close-wave-122558`，`/tmp` 与镜像 `cmp` **IDENTICAL**（换代前 `/tmp` 那份**不存在**、镜像是 **9-18 旧代** ⇒ 已一并刷新）。
- 🆕 **新登记 `D-G110`（产品缺陷 · 同因的第二落点）**：`has_ewmh_wm()` 的**第二个调用点** `win32_core.c:653`（窗态：最大/还原）**同族同病**——WM 已死＋属性残留时它 `return`，去等一个**永远不会来的** `ConfigureNotify`（该处 `:658-666` 的"无 WM 退化支"**走不到**）。**实测**：红腿含 `site=wmstate` **×2**（最大＋还原）；修前 stderr 有 `APPLY_WM_STATE … maximize=1`，修后变 `[WMCK_DIAG] … site=wmstate mode=1 branch=fallback target=0,0,1280x1024` 且**真的动**。**与 `TASK-0109` 同因同修、随 `#53` 落地**（**不许**读成两处独立改动）。
- ✅ **`D-G105`（装置 · 复用外部 X 只验"连得上"不验几何）→ 已修**：W126A 按主控授权加**几何守卫**（`XREQ_GEOM=1280x1024`，两处复用点 `[0]`＋`x_recheck_alive()`，三态 `X-REUSE=reused|skipped-geom-mismatch|self-started`，不自起在**被占号**上）；**成对读数**：假红那趟 `Windowing.Tests ❌ 失败 3/44`（复用 `:185`（1024x768））→ 守卫后 `✅ 44/44`（复用 `:97`（1280x1024））；改写记在 `docs/WAVE52-PREREGISTRATION.md` §R1（只追加）。
- 🆕 **`D-G108`（发布完整性 · `git add -A` 夹带）**：`#52` 推送时某车道用 `-A` 形态把**另一条在办车道的 native 源**扫进同一笔（`37def7e`），事后 `revert` 还原发布态（`1890b00`）；**主控现场复核**：`R` 里的在办修法**完好**、远端那三件已回冻结值 ⇒ **只污染发布态、未毁在办工作**。**过程教训（主控记）**：**推送窗口与"别人正在往 `R` 落产品改动"的窗口必须互斥** ⇒ `#53` 的派单里已加**两道防线**（逐径 `add` ＋ `porcelain`／`diff --cached --name-only` 逐件计数贴报告）。
- 🔖 **`D-G103` 追加实例（不新号）**：`#53` 期间又出两条**自伤**（都是"按名字/关键词认进程"族）—— ① 一条车道 `pgrep -f` 自匹配（只列进程、无危害）② 一条车道 `pkill -f 'mem.log'` 打到**自己的 shell**（影响面只有自身，别的车道完好，此后一律按 PID）。**两条都由本车道如实留档**。
- 🧾 **`docs/WAVE53-PREREGISTRATION.md` 由车道 W133A 在收尾链中补建（主控授权）**：缺它 ⇒ `verify-all` 第 `[14]` 步 `prereg-absent` ⇒ `NOINFO rc=2` ⇒ 冻结器的 green 断言过不去（**判据件口径**：`verify-all.sh:207-221` 用 `grep -qE "^#+ .*${gen_decl}"` 在**标题行**里找本代号）。⚠️ **本波判据先写于 `W131A` 的 `$HOME/w131a/criteria.md`** ⇒ 该件**只做波级记录与指针、不发明新判据**（件内已逐字声明）。
- ⚠️ **主控自纠一处（本条只报不改）**：我在 `T/20` 裁定里判 W133A 引的"冻结器 `w27-freeze.py:454`"为 `NOINFO`，**理由是错的** —— 我当时只 `find $R`，而**冻结器历来在仓外**（`$HOME/w21-verify/w27-freeze.py`，我复核时现场 `sha16=5acac47065f6a7aa`、`mtime 2026-09-23 12:49:07`，**与该车道报的 `101db5306e1669fe` 不符**）。**实质结论经我逐字复核成立**：该件 **`:486-487`** 确有断言 `assert re.search(r'\*\*`gen` 收官起 = nstep 步\*\*', vh)`（要求 `verify-all.sh` 头注释出现「**`#53` 收官起 = 27 步**」）⇒ 车位号 `:454` 与哈希都对不上，**判词仍按"实质成立、引证需更正"记**。
- ⚠️ **`NOINFO`（逐条，未猜）**：`pf` 非确定性根因｜`tline` 日志变化的归因｜`D-G83` 四格与真应用拖动**重测**｜`:97` 的所有者｜`UNEXPECTED=6` 逐条归因｜`~/wfp-runs/arms23/*.log` 默认 `OUT` 就是归档目录本身 ⇒ **每代重取都会再造一次同 inode**（W133A 本波已按 W31C 做法改成真副本，但**默认值未改**）。
...
- 🔴 **`D-G98` 追加「归因终局（`N3`）」段**（逐字要件已落在 `samples/WpfFeatureProbe/KNOWN-DEFECTS.md` 的 `D-G98` 段；上面判词与各条追加**一字未动**）：**发起方 = `wpfgfx_cor3.so`（MIL 桥／合成器）在**自己那条 X 连接（`fd=135`）**上发的**裸 `ConfigureWindow`** —— 线上首字节逐字 `0c 02 05 00 04 00 c0 00 0c 00 e0 00 00 05 00 00 00 04 00 00`，解码 **`win=0xc00004 mask=0x000c(CWWidth|CWHeight) w=1280 h=1024`**；调用链 `writev ← libxcb ← xcb_writev ← _XSend ← _XReply ← XSync@libX11.so.6 ← wpfgfx_cor3.so+0x131b36 ← +0x1339f5 ← +0x16fa4a`；时刻 = **WM 真还原成 `800x600` 之后 `+85 ms`**（`13.686` 发出、`13.692` 落地，**一次往返 `6.2 ms`**）。**四类候选**：**(b) 证成**（托管侧又一次几何申请，**但桥直连 X、不经 `SetWindowPos`／`MoveWindow` 导出面**）／**(a) shim 排除**（附 class-(a) 正对照）／**(c) WM 排除**／**(d) 别的客户端排除**（逐窗 `_NET_WM_PID` 普查）。**配对零反例**：有台账且未切的 **11 腿** ⇒ **红 `5/5` 有、绿 `6/6` 无**（Fisher 单尾 **`p = 2.2e-3`**；含 4 条作废腿 `7/0/0/8`、`p = 1.6e-4`）；**判别变量 = "桥这一趟到底发没发"**（绿腿一次都不发、红腿发两次：`+85 ms` 与 `≈+700 ms`）。**因果级反极性**：线上把命中请求换成**等长 `NoOperation`** ⇒ **切中真请求的 5 条里 4 条转绿**（末态 `800x600`、**第 4 跳消失**），同臂未切 **红 `8/10`**；第 5 条 `CUTP-01` **作废**。**符号级 cut 均无效 ⇒ 预登记 `P1` 被证伪**（如实记）。**旁证**：纯 X 最小客户 `xmaxrepro` **`48/48` 复现不出**。
- ★**取代声明（本条与 `§15m`／`§15r` 的关系，一句写清）**：**`K3` 段与本条不冲突、也不互相取代** —— **`K3` 排除的是"提示族"**（那份"过期尺寸约束"**从来不存在**；该句已由 §15r 声明**作废**），**本条给出的是"桥侧第二次申请"**；本条**补上的正是 `K3` 自己写下的那格 `NOINFO`**（"那 `0.1 s` 内**是谁**发的申请"）⇒ 两段合读 = 「**提示族被排除 ∧ 桥侧重发被证成**」。⚠️ **`K3`／`§15m` 原文一字未动**。
- 🆕 **新任务 `TASK-0210` [Next] ✅ **已办（`#54` 冻结内：`src/WpfGfx.Linux/Interop/MilPresentation.cs` 接窗路径只读尺寸不写几何 ⇒ `bridge feef049e9d0e313a → 4e25e4b27d4d5ae1`，同尺寸 5,028,208 B）** —— 号不撤（`ROUTES.md:749` 口径）「桥侧几何重发／接管」**（已逐字落在 §13 树的 `TASK-0111` 行下）：判定点 = **`wpfgfx_cor3.so` 在 WM 还原后 `+85 ms` 用裸 `ConfigureWindow` 把窗改回最大化几何**（`src/WpfGfx.Linux/**` 那侧的**桥／合成器**代码，**本件不定位到行**）；**判据先写**（修后该请求在红腿消失／不再改回最大化几何 ＋ `M1:R2` 臂红率显著下降且**趟数与功效先写** ＋ **协议层读数组件在位**）；**反极性** = 复原 ⇒ 红回来。⚠️ **产品改动 ⇒ 排在波 `#53` 冻结之后**（守"一条产品改动 → 冻结一次"）；**现场读数** = `#53` **已冻结并推送**（`docs/CURRENT-STATE.md:9` = `gen=#53`；§15s）⇒ **该前置已满足**。
- 🔁 **`TASK-0111` 指针（状态位未改）**：其"**下一步 = `N3`**"**已完成**（**写者已归因**，不再是 `NOINFO`）⇒ 后续落 **`TASK-0210`**；⚠️ **`TASK-0111` 状态位仍 `🔴`**（**产品侧未修**）—— §15i／§15k／§15m／§15r 的登记**一字未动**。
- 🆕 **新登记 `D-G111`（装置／纪律事故 · 仪器把用例跑到"用户真实会话的显示"上）**：车道 **WC03** 的 `batch.sh` **字段解析错位** ⇒ `DISPLAY` 被读成 **`:0`／`:1`（GNOME 会话）** ⇒ **`9` 趟真应用（`800x600` 的 `HandyControlDemo`）带真点击（每趟 `3` 次）跑在用户桌面上**（约 `25 s` 后**按 PID** 收掉）。**危险方向 = 污染用户真实桌面**（抢焦点／覆盖窗口／**不可预测的点击落点**）。**处置**：**作废留痕**（`~/wc03/run/VOIDED-*/`**不删**）＋ **按 PID 收干净** ＋ ★**`leg.sh` 加显示白名单硬闸**（非 `:18x` ⇒ `REFUSE_DISPLAY=… rc=9`，**不启应用不点鼠标**）；**本件现场复核**：硬闸逐字在位（`~/wc03/bin/leg.sh:30-31`）、留痕目录 `3` 个（`VOIDED-b4-on-display-1` **9** 项／`VOIDED-b5-on-display-1` **2** 项／`VOIDED-b5b-overlap` **3** 项）。**`NOINFO`（逐字保留）**：**那些点击落在谁窗口上证不到**。**口径句（逐字）**：「**任何跑应用的腿，必须在"入参处"硬闸显示白名单（非私有 `:18x` 直接拒跑）；不许只靠"脚本里写对了显示号"。**」⚠️ **与 `D-G103`（`pkill -f` 自杀式误杀）同族**（都是"**装置的手伸出了私有沙箱**"）**但判词不同** ⇒ **独立立号 ＋ 交叉引用**（那条伤**别人的私有批次 ＋ 自己的证据**，本条伤**沙箱之外的真实环境**）。
- 🆕 **一条方法学口径 ＋ 三件可复用仪器（并入 `D-G107`，不新号；理由先写后判）**：**要断言"进程没有发某个 X 请求"，必须有 syscall／协议层读数组件；只信 `LD_PRELOAD` 符号 hook 会被骗** —— 现场证据 = 命中链是 **`XSync@libx11.so.6`** 而**不是**插桩库（同一条日志里 shim 的链却是 `XFlush@xwrap.so+0x41`，**被插桩**）⇒ 桥到 `libX11` 的解析**绕过预加载**（最像 `RTLD_DEEPBIND`，**机制 `NOINFO`**）；车道 WC03 一度据此**错误地**得出"进程内没有任何几何申请"，是 **syscall 级 `PROTO` 台账**把它翻过来的。**并入理由**：`D-G107` 要件② 已写死"**取值时射程必须按该对象的真实结构定义**"，本次坏的正是同一要件的**"测量层级"一侧**（**射程不足 ⇒ 错误否定**），与该条危险方向（**漏判**）同向；⚠️ **`D-G104` 被机械排除**（那条是"**打了但语义被读错**"，本次是"**根本没打出来**"）。**可复用仪器（sha16，本件现场复算逐位相符）**：`~/wc03/bin/xwrap.so` **`b5c1c1dbc4c0f13d`**（符号级＋协议级双台账＋两类 cut）｜`~/wc03/bin/xobs` **`fb3fe379eb943994`**（server 侧真值时间轴）｜`~/wc03/bin/xmaxrepro` **`a058dad4a6691c15`**（纯 X 最小 EWMH 客户）。
- ⚠️ **本波前提与派单书不符（如实记）**：派单书写"冻结 `#52`…远端 head 应 ≈ `33df6aa1…`"且"`#53` 正在被另一会话跑"；**现场读数** = `#53` **已冻结并推送**（§15s）、`D-G110` **已占**、`declared` 基线 = **`146`（不是 `145`）** ⇒ 本件**按现场为准**（新号 **`D-G111`**、新 TASK **`TASK-0210`**），且 **`#53` 的在办件（`verify-all.sh`／`WAVE53-PREREGISTRATION.md`／`known-red.json`／`src/**` 三件 native 源／`wave-audit.log`／`arm-logs/tline.log`／`gen/t2d-*`）一件未碰、一件未 `add`**。
- ⚠️ **`NOINFO`（逐条，未猜）**：**桥为什么只在此臂重发**（`M1:R2` 88.9% vs `M2:R2` 9.1% 的臂间差成因）｜**桥那条连接为何绕过 `LD_PRELOAD`** 的机制｜**事故里那些点击的落点**（不可复原）｜「全 `$HOME` 还有几处装置不硬闸显示号」／「还有几处仪器只靠符号级 hook 做否定断言」**均未逐处枚举**。
...
- ✅ **车道 WC05 交件（**只读**：样本应用 `0` 趟、重活槽 `0` 次、`$R` **零写入**、未起 X、未重编任何件，用时 ≈57 min）**：四条判词 = ① **`ANCHOR=CONFIRMED`** ② **`SIADDR_READER=OK`**（两极化校准四格全区分、不粘滞）③ **`siaddr=0x0` 与指令语义"不符"那句已判死**（= **`#GP`**，不是 null 解引用）④ **"第 0 格停止点"已定性**（`W071` 的 `STOP-0` = `tid=1` 的**非致命** `SIGSEGV`，在 `/memfd:doublemapper`，`SetMatchPackageSignature…FromRegistry()[QuickJitted]+0x47`，**与崩溃链无关**）⇒ **「静默 SEGV 与 `134` 族异源」站得住**。
- ✅ **锚点独立复算 = `CONFIRMED`**：`PC − 装载基址 = `0x11df3``，该处逐字 **`48 8b 40 38  mov 0x38(%rax),%rax`**；**更强** = 回溯 `#1 = PostMessageW+0xc7` 与**调用点** `12c62: call d560 <wpf_queue_push@plt>`（返回地址 `0x12c67`）**逐位吻合** ⇒ **崩点确为 `win32_msg.c:804`**（**主控现场复核**：`nm -D` 现算入口 `wpf_queue_push=0x11cf0`／`PostMessageW=0x12ba0`，`0x12ba0+0xc7=0x12c67` 自洽）。⚠️ **口径句（写死）**：「**凡"符号＋偏移"必须写明它是哪个 `sha16` 构建的**」—— 红臂 `abf6879c027c5e73` 段 = `0xc000`／`0xf000`、权威件 `a6365183fa6d26b9` = `0xd000`／`0x13000`，**`0x11df3` 只对红臂成立**（**权威件里 `wpf_queue_push` 在 `0x14880`、`+0x11df3` 落在 `ShowWindowAsync`**）。
- ✅ **「`siaddr=0x0` 与崩点指令不符」判死（原"不许把 `siaddr` 当依据"那句现在可以升级为"依据已解释清楚"）**：**`#GP`（非规范地址）⇒ `si_code=0x80(SI_KERNEL)` 且 `si_addr=0x0`**；真·地址 0 的 `#PF` ⇒ `si_code=1` ⇒ **判别量是 `si_code`，而它从未被采**（并入 `D-G107` 第五个实例，**不新号**）。**独立铁证（不依赖 gdb）**：内核日志里红臂那族的 `.NET Finalizer` **全部崩在同一 ELF vaddr `0x11df3`**，其中 **`9` 条 `general protection fault … error:0`**（**主控现场复核** `grep -c` = **`9`**）；**gdb 趟没有内核行是机制**（`ptrace` ⇒ `unhandled_signal()==0` ⇒ `show_signal_msg()` 提前 return）。
- ⚠️ **一格**口径加固建议**（**尚未落进口径句；那行一字未动**）**：现有第四支（"`gdb.txt` 里存在 `W118A-STOP-N`（`N≥1`）且 `signo=11`"）**不要求它是最后一条停止点** ⇒ 原理上**可被一次非致命 `SIGSEGV` 满足**（`W077` 正是**只能靠第四支入册**的趟）⇒ **建议加固为「第四支 ∧ 该 `SIGSEGV` 是最后一条停止点（其后进程消失）」**；**是否加固 ＋ 三批计数要不要重算** ⇒ **待车道 WC06 交数后由主控裁定**。
- 🆕 **根因链（每一环都有行号／字节锚点；逐字落在 `KNOWN-DEFECTS.md` 的 `D-G109` 🔁 追加段）**：**悬垂 `w->owner_thread` 的 UAF** —— 属主线程退出 ⇒ TLS 析构 **`wpf_thread_destroy`（`win32_core.c:147-156`）`free(t)`** 而**不摘 `g_wpf.threads`、不清窗口 `owner_thread`、不持锁** ⇒ `.NET Finalizer` 走 `HwndWrapper::Finalize()` → P/Invoke → **`PostMessageW`（`win32_msg.c:779` 取 `w->owner_thread`，`:801` 已是 UAF 读）** ⇒ **`:804 wpf_queue_push`** ⇒ **`:71/:73` 从已释放块读 `tail`／`head`**；**glibc 2.35 `tcache_entry{next@+0,key@+8}` 与 `wpf_thread{self@0,head@8,tail@16}` 同址** ⇒ `head` = `tcache_key`（随机、进程内恒定、**非规范**）⇒ **`tail==NULL ∧ head!=NULL`** ⇒ 进 **`:74-82` 自愈分支** ⇒ **`#GP`**；**非空队列退出** ⇒ 写 `tail->next` ⇒ **`error:6`**（= 内核那批 `error 6` 记录）⇒ **`key` 进程内恒定亦解释"两趟 `PC` 与 `si_addr` 逐位相同"**。
- 🔴 **`TASK-0209` 修法四要件（主控已定，逐字；状态位仍 `🔴` —— 产品侧未修）**：**① `wpf_thread_destroy` 先 `wpf_lock()`；② 把 `t` 从 `g_wpf.threads` 摘链；③ 把它名下窗口的 `owner_thread` 失效（或给 `wpf_thread` 加"已死"标记且永不 `free`）；④ `PostMessageW` 取 `owner_thread` 后加存活校验**。**禁止"只在 `wpf_queue_push` 里加空指针守卫就收工"** —— UAF 会留着，崩溃点只会漂到 **`wpf_queue_wake` 的 `write(t->wake_write,…)`（`win32_msg.c:90`／`:106-112`）**。⚠️ **"有没有第五条／哪一条是最小充分修法"由车道 WC06 独立裁定**。
- 📌 **本笔的调度（写给下一个会话）**：**两条车道已派** = **WC06**（重判全部历史批次 ＋ 补 `SI_CODE`／`STOP-0` 两格 ＋ **`1` 趟 `watch` 诊断趟**钉"那块已释放内存被谁复用" ＋ 修法第 3/4 条裁定；总趟数 ≤3）／**WC07**（**私有副本**路线：`cp -r $R/src/WpfGfx.Linux.Native ~/wc07/native`（**3.2 MB，`build-shim.sh` 自包含、不需要仓库上下文**）⇒ 实现修法 → **确定性触发**（次级线程建窗后退出再从别的线程 `PostMessageW`）→ 两极化 ＋ 零回归 ⇒ 交 `task0209.patch`，**绝不写 `$R`**）。**为什么产品改动仍不落 `$R`**：另一会话的**落仓器进程现场已死**（`1383190`／`1388625` 均 `DEAD`，`~/w136a/logs/landing.log` 最后一条停在 `18:41:41`）⇒ 权威树**此刻无写者**，**但它一旦被拉起就会 patch `src/` 三件 native 源并重建 ⇒ 越过它写产品件就是 `D-G108` 的形态** ⇒ **本件与 WC07 都不碰 `$R/src/**`**。
- ⚠️ **边界／`NOINFO`（逐条，未猜；与 `KNOWN-DEFECTS.md` 同读）**：**"复用者／tcache 状态"**（只有 `watch` 趟能钉）｜**"样本里究竟是哪个线程先退出、为什么它的窗口还在"未直接取证**｜**内核 `0xf000` 族 Sep 20／22 那几次是否与红臂件逐字节同构件**（只能按段布局归族）｜**`wpf_window_find` 的 HWND 复用候选**（现有证据打不了）。**该车道自伤三条**（`-O1` 把一格优化掉／`awk` 无 `strtonum` 静默空输出／手算十六进制错一次）**逐字见 `D-G109` 追加段**。

- 🔁🔁 **车道 WC06 交件（`#53` 冻后第三批 · 第十八笔，2026-09-23；报告 `~/wc06/WC06-report.md` **FULL `7c5783b96aa66239…`**／去自引用 **`da452fc349f5a4d3`**／**531 行／40,616 B**，判据先写 `~/wc06/criteria.md` **`5597773f189af246`**；**`$R` 零写入**、未重编任何件、未改别人证据目录一个字节；真应用 **1 趟** ＋ 校准 3 趟（`T-CAL1` 作废）；**21 min 11 s**）**：逐条落在 `KNOWN-DEFECTS.md` 的 `D-G109` 第二个 🔁 段（**不新号**；上面 §15u 各条**一字未动**）。
- ✅ **重判 `RECOUNT=OK`（独立于任何汇总行）**：`w128a` `175` 趟 ⇒ **2**（`W071`／`W077`）；`w98a` `128` 趟 ⇒ **1**（`L1B024`，**只靠 `rc139` 支**）而**该批台账自身的 `hit139` 列写 `0`**；`w118a` `64` 趟 ⇒ **0**（另 `NEARMISS` 6）；上界复现 `2/175 ⇒ 3.55%`／`2/236 ⇒ 2.64%`／`3/235 ⇒ 3.27%`。**主控现场复核**：`~/w98a/runs.tsv` 的 `L1B024` 行 = `hit139=no` ∧ `cand139=yes` ∧ `rc=139` ∧ `verdict=crash-segv`。
- ⚠️ **两条**批级**读器假读数（并入 `D-G107` 族，不新号）**：**① `NDEEP` 兜底 `0`** —— `~/w128a/bin/one128.sh:155`，根因 = `gdb.cmds:48-50` 的 `alive()` 把 `PYERR` 串当"还活着" ⇒ `:134` 抛未捕获异常 ⇒ `:200` 汇总行永不执行 ⇒ **`w128a` 159/175 趟、`w118a` 46/75 趟把"有深停＋两份栈"写成"0 次深停"**（**主控现场复核**：`~/w128a/run/` 含 `gdb.txt` 的 183 个目录中 167 个无汇总行、其中 **159** 个确有 `W118A-STOP-` 行）｜**② `PC0` 的 `${DEEPIDX:-0}` 退化**（`one128.sh:163-164`）⇒ `w118a` **8** 趟把**良性 .NET 自产 `SIGSEGV`**（`memfd:doublemapper`）的 `PC` 写成"崩点 `PC`"。
- ✅ **口径加固判词 = `HARDEN=ADOPT(no-op)`（先写后判）**：逐字证据 = `W071/gdb.txt:54`（最后停止点 `signo=11 tid=6`）其后 `:78 Program terminated with signal SIGSEGV`；`W077/gdb.txt:55` 同款、其后 `:78 CONT-3 ok=no | PYERR` ⇒ **不损失真命中**；但 `R1` 在三批上**没挡住任何一趟**（173 趟是被 `STACKOVF==0` 挡的）⇒ 记 `ADOPT(no-op)`（**既不是"该加固"也不是"不该"**）。**附**：`n_banners > n_stops` 恰 **5** 趟（`B101`–`B105`）⇒ `N≥1` 闸**落空**、属**装置伪影**（进程还活着就被 `g("kill")`），`~/w118a/STATUS.md:40` 的作废**独立复核成立**。
- ⚠️ **根因前提被**收窄**（本笔最要紧的一条；取代 §15u 那条"根因链落定"的**结论强度**，不改其文字）**：车道 WC06 用**更硬的直接判据**（`wpf_thread_destroy` 记 free 集合 ＋ `wpf_queue_push` 入口查 `$rdi ∈ FREED`，**不需要等那 1.14% 崩溃**）跑真应用 ⇒ **`PUSHN=558`、`TDESTROYN=0`、`UAFPUSH=0`、`FREEDN=0`**；仪器**先校准**（`T-BPCHK` ⇒ `TDESTROYN=1`）⇒ **"悬垂 `owner_thread` 的 UAF"在这条跑法里缺少前提**（只有主线程建窗 ⇒ 其 TLS 析构在**进程退出时**才跑 = 崩溃之后）。**边界逐字**：只对"可复现 8 击跑法"成立；`W071`／`W077` 那两趟的 destroy 次数 = **`NOINFO`**。
- 🆕 **新方向 ＋ 修法裁定（车道 WC06 独立判）**：`head != NULL ∧ tail == NULL` 在现源码下（`pop` 的 `t->tail = prev` 已修、写入点都持锁）**没有内存损坏就不可达** ⇒ 它本身就是"**`t` 被写坏**"或"**`t` 不是 `wpf_thread`**"的证据 ⇒ 下一趟（**1 趟、不需崩溃**）在 `wpf_queue_push` 入口做 **`t->self == t`** 自校验 ＋ 普查"`t` 是否在 `g_wpf.threads` 链上／命中的 `wpf_window` 是否还在 `windows` 链上"。**修法三件不够**：`:779` 取 `target` 后 **`:780` 立刻解锁**（`:801-804` 全在锁外）⇒ 持锁堵不住；摘链只解决 `:815` 且**恰恰让 `owner_thread` 悬垂** ⇒ **第 4 件（最小充分）= `wpf_thread` 永不 `free` ＋ 死标记 ＋ 使用点判死 ＋ fd 失效（`wake_*=-1`）**。**禁令更强理由**：`if (!p) return;` **连触发都不会触发**（`head` 是**非规范的**非 `NULL` 值）。
- 📌 **调度（本笔）**：**WC07**（私有副本修法＋确定性触发）已按新证据**改口径**（不许声称"修好了 `D-G109`"，触发升为第一交付）｜**WC08 已派**（`~/wc08/`：`wpf_queue_push` 入口**异常普查**（558 次 push／趟，**不需崩溃**）＋ 按 `W071`／`W077` 的 `pad=1129`／`1226` **定向复现**（上限 **90** 趟、分块 15、**命中即停**）＋ 命中现场十六格；**总趟数 ≤ 96**）。**两条 LANE 都不碰 `$R/src/**`**（另一会话的落仓器一旦被拉起就会 patch 三件 native 源并重建 ⇒ 越过它写产品件就是 `D-G108` 的形态）。
...
- ✅ **车道 W136A 已把同族修法落进 `$R`（`20:49:21`）**：`win32_core.c` **`c66528843de4a370`**／`win32_msg.c` **`12175591b736bb3f`**／`win32_internal.h` **`c13390de6f870999`**／件 **`2067cb1c97728791`**；形态 = `wpf_thread_destroy` **锁内摘 `g_wpf.threads`**（`:162-168`）＋ 清**窗口与定时器**的 `owner_thread` ＋ `head/tail` 归零 ＋ `free(t)`；`PostMessageW` 对"窗口在表里但 `owner_thread == NULL`"⇒ **`return 0` ＋ `ERROR_INVALID_WINDOW_HANDLE`（`1400`）** ＋ **无条件** stderr 台账 `[POSTMSG_DEAD_TARGET]`（`WPF_POSTMSG_DEAD_MAX 32`、`static ⇒ 导出仍 547`、**不受 `MSGFLOW` 开关限制**）。**"永不 `free`"政策裁定（主控）**：**不强制** —— `grep` 现场枚举 = 全仓持有 `wpf_thread*` 的字段**只有四处**（窗口 `owner_thread`／定时器 `owner_thread`／`t->self`／`threads` 链），F3/F3b **已清三处**（`self` 是自指、无意义）⇒ **已无悬垂持有者**；WC07 的 `P4`（永不 `free`）是**更强但非必要**的 belt-and-braces（代价 `64 B/线程`），**留作可选**。
- ✅ **第三方**确定性触发**（本缺陷的"可判定判据"由此成立；WC07 交件）**：载体 = **合法 Win32 API 序列**（次级线程建窗 → 线程退出 → 另一线程 `PostMessageW`）；**红臂 `a6365183fa6d26b9` `12/12` 崩**，12 趟签名逐位相同 = `rip_off 0x14983`（`wpf_queue_push`＋**259**）＋ 逐字 `14983: 48 8b 40 38` ＋ **`si_code=128`（`#GP`）** ＋ **`si_addr=0x0`** ＋ `%rax` 每趟不同的**非规范随机值（= glibc `tcache_key`）** ⇒ **`t` 是一块已释放的 `wpf_thread`**；**绿臂 `ce37e26257261cdb` `0/12`**；**`$R` 现行件 `2067cb1c97728791` `0/12`**（12/12 `rc=0 errno=1400`）。**正对照** `LIVE` 红绿各 `6/6 seen=1`；**台账**修前 `0` 次/修后 `12` 次。⚠️ 该装置是**第三方写的** ⇒ **建议直接当 `#55` 的验收证据**（并要求第四个独立复核：已派车道 **WC08** 在两臂上各复核一次）。
- 🆕 **残余缺陷已立号 `TASK-0211`**（两处窄 `TOCTOU`：`PostMessageW` 解锁在 `push` 之前、`PostThreadMessageW` 同形；**可利用性 `NOINFO`**；**产品改动 ⇒ 等现行未冻结改动冻结之后、由该文件在办线落地**）。
- ✅ **三项裁定（主控，逐字）**：**(A) WC07 的 `task0209.patch` 不落库** —— 与 W136A 现行件**冲突**（`patch --dry-run` **1/5 hunk FAILED**）；它留作**独立第二实现**（其 `P2` 把 `unlock` 挪到 `push` 之后、`P4` 永不 `free`），**不作为修法本体**。**(B) 口径修正（先写先登记，防后人踩）**：`[POSTMSG_DEAD_TARGET]` 是**无条件** stderr 台账 ⇒ **`APP_TEXT_BYTES ≠ 0` 不再等于"应用自己说话了"** ⇒ `SILENT_SEGV_HIT` **第一支**自本行起改为"**剔掉具名台账行（`[POSTMSG_DEAD_TARGET]` 及其续行）之后**为 0 字节"，**且两个数都要打印**（对已观测三批无影响 —— 那三批用**修前件**、不打这行）。**(C) `TASK-0209` 状态位保持 `🔴`（不许转 ✅）** —— 修法已落树但**未随波冻结、未过任何门禁**，且本号的**偶发签名（`1.14%`）是否同因仍未闭环**。
- ⚠️ **仍未闭环（逐条，未猜）**：① **偶发签名是否同因** ⇒ 判别量 = "**崩掉的那一趟里到底有没有 `wpf_thread_destroy`**"（WC06 在**不崩的**趟里实读 `TDESTROYN=0`；WC07 的 `t->head = tcache_key` 又**指向**同因）⇒ **车道 WC08 正在打这一格**｜② **"谁写坏 `next`"未取证**｜③ 两处 `TOCTOU` 的**可利用性**｜④ `134` 族在两趟对照里**未复现**（`NOINFO`）。
- ⚠️ **WC07 自伤四条（如实，逐字见 `D-G109` 追加段）**：首次构建**违规没走重活槽**｜装载基址取错一次（差点把"崩点不在 shim 里"当结论）｜其第一版修法**把 `owner_thread` 置 NULL ⇒ 被既有"无属主"兜底接走 = 静默丢件**｜触发件 `LIVE` 首版漏覆盖消息号 ⇒ `6/6` 假 `seen=0`。
...
- ✅ **`#55` 已冻结**（`docs/CURRENT-STATE.md:9` = `BASELINE-FROZEN gen=#55 sha16=38320d5e377a0dc8`；冻结块自述逐字："**本波零接线**（`verify-all` 仍 **27** 步）"）⇒ **`TASK-0209` 的修法（F1 零解引用隔离／F2 具名台账 `[QUEUE_CORRUPT]`／F3 摘链／F3b 清 `owner_thread` ＋ 主控裁定 (A)`[POSTMSG_DEAD_TARGET]`／(B)**拒绝投递** `return 0`＋`1400`）已随波冻结**，`win32shim = 2067cb1c97728791`。
- ✅ **`D-G109` 收官（车道 WC08 三轮；报告 `~/wc08/WC08-report.md` 573 行／60,931 B／去自引用 `fb026b0a58184aa9`；判据**先写后跑**三对）**：① **首次真应用复现**（`W8AA-04`，十六格，与 `W071`／`W077` **七格逐字相同**）② **`UAFPUSH = 1` = UAF **直接判据**（已 `free` 的 `wpf_thread` 被 `wpf_queue_push` 当队列读）③ **`G1' = CLOSED`**（非主线程建了 `wpf_thread` **并且它退出了**）④ **`ITER = FIRST`** ⇒ 排除"只是末端 `next` 被写坏"⑤ 问 1 `LEGIT-WRONG`／问 2 `ANSWERED`（来源 = **修前 `wpf_thread_destroy` 既不摘链也不清 `owner_thread`** ⇒ **`#55` 的 F3／F3b 正好堵这两条**）。
- ⚠️ **同批一处**修正**（必须与上条合读）**：**`WC08-THREADS len_max=2 first_over1=True` 在 10/10 趟成立** ⇒ **"非主线程建了一个 `wpf_thread`"是每趟都有的、不是崩趟特有** ⇒ **真正稀缺的是"它退不退出"**（合并 `1/17 ⇒ 5.88%`、单侧上界 `25.0%`、Fisher `p=0.235` ⇒ 两臂区分不开）⇒ 上一轮自标的 **`WEAK` 保持**；诊断趟**不进任何分母** ⇒ **`2/175` 口径不变**。
- ✅ **`G2` 定案（本件对 `#55` 预登记的措辞修正，已转达那边）**：现场形态 = **`t->head` = 非规范 `tcache_key` ∧ `ITER=FIRST`**；**不是**预登记 §1 的 `head = 0x102`（那形态 `SI_ADDR = 0x13a`）⇒ §1 那句只能读作"**`repro`（`MALLOC_ARENA_MAX=1` 钉死 arena）腿的形态**"。
- 🔴→✅ **`TASK-0209` 状态位改 `✅`（**限定**，行锚定改法：只改状态位 ＋ 追加一句限定，判词文字不动）**：该 ✅ **只指"产品修法已随 `#55` 冻结（含门禁）＋ 『写坏者』已定性（修前 `wpf_thread_destroy` 不摘链／不清 `owner_thread`）"**；**残余** = **`TASK-0211`**（两处窄 `TOCTOU`，产品改动，待 `#55` 后由该文件在办线落地）＋ **自然发生率 `NOINFO`**（"为什么是 `2/175`"未定；冷断点 `wpf_thread_self+0x72` 已派第四轮 ≤6 趟）＋ **`ARENA_REUSE` 是主因还是仅加速器未判**。
- 📌 **`TASK-0708`（仪器波）保持 `🔴` ＋ 指针；主控决定（dated）= 本会话不落地、等 `#56` 指派**：整包在 `~/wc09/`（`WC09-report.md` `cc81f295336ee0a2`／`d2641b235e3b87e8`｜`criteria.md`｜`tooth-readings.tsv`｜`declaration-edits.md`｜`known-red-additions.json`（**只剩 1 条 `arm=uia-door`**）｜`fp-coverage-edit.md`｜`apply-0708.sh`｜`draft/uia-door-step.sh`（在册红包装层，**四档实测**）｜`draft/w27-freeze-GENS56.md`｜`draft/WAVE56-PREREGISTRATION.md`｜`draft/apply-checklist.md`），沙箱重演读数 = **`VERIFYALL_SELF=PASS names=31 decl=31 gen=#56 dup=0 order=OK prose=OK prereg=PASS`**。**接线要动的第七件在仓外**：`~/w21-verify/w27-freeze.py`（`1dda5297af0c8e2d`）的 `GENS['#56']` 条（`nstep=31`、`prev='#55'`、`green` 含四个新步名、`infp` 终值）**＋ 声明类红白名单**（真红 `UIA-DOOR` 直接进 `run_step` 会让本波冻结**当场失败**，必须走包装层 ＋ `repin-generation.py --why` 同趟）。⚠️ 主控**未碰** `verify-all.sh`／`close-wave.sh`／`known-red.json`／`~/w21-verify/**`（零写入自证）。
- 🧾 **仪器／纪律自伤并入既有号（**不新号**；正文落 `KNOWN-DEFECTS.md` 的 `D-G103`／`D-G107` 段，本件只登记指向）**：① 车道 **WC08** 的清场循环按 `/proc/*/cmdline` 匹配 `wc08` ⇒ **把执行它的 shell 自己也杀了**（`D-G103` 同族；已落纪律：模式须写成不能匹配自身 ＋ 显式跳过 `$$`，此后四次清场未再自杀）② 车道 **WC09** 的"**双引号里的命令替换被 shell 执行**"两例（`$(sys.argv[1])`／裸反引号）＋"plan 里**写死额度**过期"（`D-G29` 同族）＋ `ast.literal_eval(GENS)` **不可用**（值是 `dict(...)` 调用，须按 `ast.Call.keywords` 取键）。
- ⚠️ **本笔的写域与边界**：`W142A` 现场已开工（下一波写域已有人接手）⇒ 本件只落 `KNOWN-DEFECTS.md`／`ROUTES.md` 两处**声明类**登记 ＋ **重生成** `build/MilBridge/tools/defect-registry-declared.tsv`（`DEFREG=PASS declared=147 route_ids=147`、`DECLDRIFT=0`、`DG_IDSET_IDENTICAL=yes`）；**`NOINFO`（逐条）** = `D-G109` 的自然发生率｜`ARENA_REUSE` 主因性｜`TASK-0211` 的可利用性｜`#55` 预登记 §1 的措辞由**那边**改（本件只转达）。
- 🆕 **新发现的一条跨会话污染源（本笔实测，落 `D-G104`／装置卫生族，**不新号**）**：**`/tmp` 是多会话共用区** —— 主控本次用 `/tmp/a.txt` 暂存"编号集基线"，**在落册前后被另一车道覆盖**（现场复核：该文件已变成 `regression-decision.py --cases` 的 `LEDGER ROW …` 输出）⇒ **一次校验当场假崩**（`diff` 报 111 处差异、看起来像"编号集被改"）。**纪律（逐字）**：**凡"校验/比对"用的暂存件一律落私有目录（`~/wcNN/`），不许用 `/tmp` 的通用文件名；用前必须自证内容形状（例如 `wc -l`／行首形态）再比。** ⚠️ 本条**不改**任何既有编号的值与判词、**未**把任何红写成绿。
...
- ✅ **`D-G109` 四轮收官（每环都有真应用读数）**：**Q1** = **`.NET TP Worker`**（`tid=10/11`，应用启动后 **4.2–4.7 s** 建成，三趟逐趟相同）｜**Q2** = **它退了**（`W8CC-01` 的 `WC08-TDESTROY n=1` 的 `t` 与创建指针**逐位相同** ⇒ **建—退配对成功**；合并退出率 **`2/20 = 10%`**：钉 `2/7`／不钉 `0/13`，单侧上界 ≈`25%`）｜**Q3** = **它拥有窗口**（`WC08-WIN-BY-NEWTHREAD push_n=18 w=… hwnd=0x200001 owner=…8900`；`WINBYNT=330/324` vs `win_owner_main=2126/2091`）⇒ **它一退出，那个窗口就带悬垂 `owner_thread`**。
- ✅ **"偶发"被拆成三个乘数并逐个量化**：**建线程（`100%`/趟）× 该线程退出（`≈10%`）× 退出后又被 post 且块已被复用（钉 arena 下 ≈高）** —— 机制链 = TP Worker 建 `wpf_thread`＋拥有窗 `0x200001`（本轮）→ 它退出（修前 `free(t)`／不摘链／不清 owner）→ `.NET Finalizer` 对该 hwnd `PostMessageW(0x8000)` ⇒ `head` 非规范 ∧ `tail=0` ⇒ `:79` 自愈分支 ⇒ `#GP` ⇒ 静默死（`W8AA-04` 十六格）。**仪器自证**：冷断点两极化校准（`~/wc08/cal/tlscold`，**5 次调用 ⇒ `COLDN=2`**，指针逐位吻合）＋ `SELFOFF 0xee50` 三趟全过。
- ⚠️ **两条"会造假 0"的仪器缺陷（**并入既有号**）**：**① `D-G107` 第六实例** = push 回调里**先读 `w` 后绑定**且缺 `global` ⇒ 每次 push 抛 `UnboundLocalError`，被 `stop()` 的 `except` **吞进 `ERRS[:20]`** ⇒ 该行之后**整段格子静默变 0**（`W8CC-01` 的窗口侧读数**全部作废、不许入册**；"`pushes=0`"**不是事实、是"从未测过"**）｜**② `D-G105` 追加** = `arm_cold()` 经 `new_objfile` **提前置好 `BASE`** ⇒ `if BASE[0] is None:` **整块被跳过** ⇒ **`ENV` 读取与 `BP2` 从未执行**（`ENVSEEN=None` 的含义是"**从没读过**"）⇒ **不满足 WC08 自己 §9.2／§11.3 的"缺一即该臂作废"**；22:31 版**只修了 ENV、`BP2` 那一半仍在** ⇒ **若之后报"第四轮已自证"，那是事后补的**。
- 🧾 **口径句（逐字，写死；车道 WC11 提出、主控采纳）**：「**凡"缺席即等于 0/否"的判读一律不许入库，除非该格有"本次确实执行过"的正向证据；而"每趟打印 `WC08-ERR`"／"零 push 的趟里也打出 ENV 行"这类自证，本身就是那条正向证据。**」
- 🔴 **裁定：不投"命中趟多出的那个 `wpf_thread`"**（`W8AA-04` 的 `len_max=3` vs 本轮三趟 `len_max=2`；冷断点 **1 趟**可答）—— 记 `NOINFO` ＋ 1 趟成本；理由 = **现行登记的任何判词都不依赖它，且同类重复投入已出现一次**（WC11 的第 4 轮与 WC08 第四轮撞车 ⇒ 已令 WC11 **停趟收工**，只留它的审计与自证读数）。
- ⚠️ **本轮调度与边界**：**WC11 已收**（它的两条审计已兑现，剩余工作与 WC08 第四轮重复 ⇒ 令其**立即停趟**、只交审计清单 ＋ 三件自证读数 ＋ 口径句）｜**`W142A` 在办**（下一波写域有人接手）｜**`TASK-0708` 的 `#56` 包仍在 `~/wc09/` 待指派**（主控决定：不落地）｜**诊断趟不进任何命中率分母**（`2/175` 口径不变）｜**"退出 ⇔ 必崩"仍未证**（`W8CC-01` 退出后没崩）｜**臂的选择**已在 WC08 §13.5 **开跑前公告**（② 的 `10%` 以钉臂为主；"现场不钉"的同一格未跑 ⇒ `NOINFO`）｜本笔**只落两个声明类文件**（`KNOWN-DEFECTS.md`／`ROUTES.md`）＋ **重生成** `defect-registry-declared.tsv`。
...
- ⚠️ **`Q2` 的边界更正（车道 WC11 指出、主控复核成立）**：**"它退了"当时只落在 `W8CC-01`——正是 D1 判作废的那趟**；`W8CC-01b/02` 两趟**都 `TDESTROYN=0`** ⇒ **"持窗 ∧ 退出"当时是两趟拼出来的**。§15x 与 `D-G109` 那条 🔁🔁🔁🔁🔁 的**原文一字未动**，本行**声明取代其"建—退配对"与 Q2 的强度**。
- 🆕 **"一格定罪"（主控裁定，**0 趟**）**：现场逐趟表 = `W8AA-04` **`TDESTROY n=1` ∧ `UAFPUSH=1` ∧ `len_max=3`**（**全仓唯一**一趟 `len_max=3`；其余 `len_max=2` ×12、`len_max=0` ×3）｜`W8CC-01` `TDESTROY n=1` ∧ `UAFPUSH=0` ∧ `len_max=2`｜`W8CC-01b/02` 无 destroy ∧ `WIN-BY-NEWTHREAD push_n=18` ∧ `len_max=2`。⇒ **`W8AA-04` 里只可能有三个 `wpf_thread`**：① 主线程（崩时**活着** ⇒ 其 TLS 析构**不可能**跑）② **那个非主线程**（= `.NET TP Worker`，身份由白趟冷指名给出）③ 第三个（其出现与"**悬垂 `owner_thread` 的那条 post 由调用线程 `wpf_thread_self()` 现建**"自洽 —— 正是 `#55` 的 (A)/(B) 后来堵掉的那条改投）⇒ **该趟唯一能被 `destroy` 的只有 ②** ⇒ **`W8AA-04` 单趟同时具备"非主线程退出 ＋ 其窗口留悬垂 `owner_thread` ＋ `UAFPUSH=1` ＋ 崩"** ⇒ **链条在同一趟内闭合**。
- ✅ **裁定：不投 WC11 建议的那 1 趟**（`W11A-DESTROY-OWNS-WINDOW`）—— 理由 = 上面那条枚举定罪已把致命前置落回单趟，且"命中趟多出的那个 `wpf_thread`"**同时得到机制解释**（= 第三个，由悬垂 owner 那条 post 现建）。⚠️ **并纠正"1 趟"这个说法**：**"能测"≠"能观测到"** —— 若要**直接**观测"冷指名 `t` == destroy `t` 且同趟持窗"，那是**白趟**（`p≈10%` ⇒ 80% 把握约需 **16 趟**）。
- ✅ **WC11 的第三方自证（第三个独立实现的冷断点校准，采纳）**：校准趟 `T-CAL01` **5/5 全绿** —— `W11A-COLDN 2`（校准件调 `wpf_thread_self` **5 次**）｜`W11A-SELFOFF 0xee50`｜`W11A-BPSTATE bp2=True`｜`PUSHFLOW ok=0 fail=0`｜**零 push 的趟里也打出 `W11A-ENV probe_init=True tries=1`**（上游那种写法在此**什么都打不出** = D2 的可见差别）。它与 WC08 那套的差别：**幂等 `probe_init()` 在 push／destroy／冷断点三入口都调**（治 D2）＋ 加牙（`PUSHFLOW`／`DSFail`／`PRINT-TRUNC`）＋ 加格（`W11A-DESTROY-OWNS-WINDOW`／冷命中 `bt`／hwnd→owner 归属表）。**它对 Q1／Q3 的只读复核也可立**（干净趟 2/2：`owner` 与各自 `COLD n=2` 的 `t` **逐位相同**）。它的自证件 = `~/wc11/probe/gdb_wc11.cmds.tmpl` `f0751fb79d55e42f`；判据 `~/wc11/criteria.md` `959cdbdadac98f82`。
- ⚠️ **三条新自伤（并入既有号，**不新号**）**：① 车道 WC11 扫 `/proc/*/cmdline` 判 `:91` 占用时**匹配到自己那条 shell**（**`D-G103` 同族第 4 例**：模式匹配到发出者）② `grep -rl 'wc11' "$R"` 得 **216** 命中，**实为 `libSkiaSharp.so` 一类二进制里的巧合字节**（改 `grep -F -rl 'W11A' "$R"` = **0** ⇒ 零写入成立）⇒ **口径句（逐字）**：「**在"文本＋二进制"混合树里做"某串在不在"的断言，必须先排除二进制（`-I`／白名单）或用带前缀的标记串；裸串命中一律要人工排除二进制。**」③ 未加引号的 heredoc 把**反引号内容当命令执行**、抹空 `STATUS.md` 若干行（已截断重写）⇒ 并入 `D-G103`（同族：**shell 求值**）。
- **边界**：`WC11` **应用趟 0／校准趟 1／重活槽持有 0 s**（`W11A-01` 排队中即被叫停、无产物）；**诊断趟不进任何分母**（`2/175` 不变）；**`TASK-0708` 的 `#56` 包仍在 `~/wc09/` 待指派**；**`TASK-0211`（两处窄 `TOCTOU`）仍待该文件在办线落地**；本笔**只落两个声明类文件** ＋ 重生成 `defect-registry-declared.tsv`。
