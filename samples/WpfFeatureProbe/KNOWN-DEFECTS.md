...
### `D-G70`（**产品缺陷**）
- **🆕 在册数更正（2026-09-24 车道 W154A-PTS 只读盘点，主控落册）**：**「111 条 `Fs*`/`Lo*` 缺口」复现不出来** —— 工具口径 **100**（`python3 src/WpfGfx.Linux.Native/tools/check-shim-coverage.py --tier all`，件 `libwpfwin32.so fc60c34d51fd9247`／550 导出；**2026-09-26 车道 W174A 现算**，`so`/`exports` 与 `.so` 的 `nm -D` 逐名相等）− **11 条 `#if NEVER`** − **1 条工具误报**（`FindWindowExWrapper` 的 **`EntryPoint` 名逐字已在 `.so` 里**，工具却在 `…Wrapper` 尾名上又补了一个 `W`、首候选 `FindWindowExWrapperW` 永不存在 ⇒ 误报）⇒ **可操作缺口 88**（55 `Fs*`／19 `Lo*`／6 `Nl*`／4 文本分析／4 `*Wrapper`，后者**全仓零引用**）；**实现口径 97**（88 ＋ `src/WpfGfx.Linux.Native/src/win32_pts.c` 里**已导出但恒返回 `-10000`** 的 **9** 个入口；⚠️ 旧在册定义把这一项**硬编码成 6** ⇒ 已废，改用机械定义 `可操作 + 9`）⇒ ⚠️ **旧的两条「闭合」已断**（`unimplemented.md` 的「97 缺失（93+4）」按另一套口径、`103+6=109` 已不成立）⇒ 现闭合式为 **88 + 9 = 97**（`可操作 88` ＋ `win32_pts.c` 恒 `-10000` 的 9 条）。⇒ **按 111 估工作量虚高 26%**（111/88 − 1），且旧数在 **9 个文件**里被复述（含 C 源码注释）⇒ 逐处更正排 `TASK-0720`。**并纠正 `W78A`**：`#if NEVER` 死声明实为 **11 条不是 9**（`Pts.cs:3525`／`:3779` 也在区间内）⇒ 其「真会炸 99 条」应为 **97**。**行为形状更正**：「与 `A1` 同形、恒返回 `-10000`」**只对 4 条成立**（`LsErr` 3 ＋ `int` 1）；**`void*` 3 条必须返回 `NULL`**、**`void` 5 条根本没有「返回非零」这个形状**。**`Nl*` 的性质更正**：不是「缺了会崩」，而是**上游 `catch` 吞掉的静默降级**（`NaturalLanguageHyphenator.cs` 的 ctor `catch (EntryPointNotFoundException) {}` ＋ `AnalyzeText` 的 `_hyphenatorResource==IntPtr.Zero ⇒ return null` 守卫；`SpellerInteropBase.cs` 同形）⇒ **`NlCreateHyphenator` 返回 `IntPtr` 句柄，返回 `-10000` = 假句柄 ⇒ 守卫当场失效** ⇒ **不做 stub**，按 `D-G76` 体例**在册声明为「有意降级」** ＋ 一条牙（`IsHyphenationEnabled=True` 与 `=False` **行为等价**，真实现后**必红**）。
...
- **机制定位（`2026-09-26` 车道 W177A 只读取证 `TASK-0723`，主控落册）＝ **时序**，不是"走到但被吞"**：①**谁**：`HandyControl.Controls.Window` 的 `HwndSourceHook` → `WmGetMinMaxInfo`（`Window.cs:333-341`，实参 `SHAppBarMessage(4=ABM_GETTASKBARPOS)`；`InteropMethods.cs:485-486` 的 `DllImport("shell32.dll")`）；同族第二处 `WindowHelper.cs:142-143` 由 `WindowState==Maximized` 守卫 ⇒ **启动期不走**。②**何时**：hook 由 `OnSourceInitialized` 才挂（`Window.cs:261-264`）；shim 全仓 `WM_GETMINMAXINFO` 派发点**只有两处** —— 建窗期（波 58，`win32_core.c:1034`）与**首次 map 之后**（波 `#50`·`D-G83` 修法 H1，`:1195`，件内字串 `after-map`）。③**为什么修前不抛**：`#50` 前**没有** map 后那一拍 ⇒ 应用那条语句**根本没走到**（`UNREACHED`）；而 shim **从未导出** `SHAppBarMessage`（`win32_oem.c:123` 的 shell32 段只补 `SHGetFileInfo*`/`ShellExecuteW`/`SHCreateItemFromParsingName`）⇒ 走到即抛，被 demo 自己的 `DispatcherUnhandledException`（`App.xaml.cs:76`，`e.Handled=true`）接住 ⇒ **每次启动 242 B、进程不死**。
- **判别的关键腿（两臂只差 shim 一件，95 件里逐件核过只差 1 件）**：**装符号 ⇒ 判词翻动**（该行消失、字节恰 **−242**，`diff` 其余 diag **逐字节相同**）⇒ 调用点**可达**、唯一失败原因＝**缺符号**；**`B1` 装符号后桩 0 次／查询 0 次 ⇒ "没走到"**（"吞"必先有"调"）；**`C0`（`#50` 前一支，四导出尺寸 `318/433/285/434` 与 `ROUTES` 所记 `#50` before 逐位相同）交付 5 次、`after-map` **0** 条、查询 0 次，窗口**照样可见** ⇒ 成因钉在"**map 之后那一拍**"，**不是**"窗口可见"**。**件级相关 5/5**：`after-map` 在场 ⟺ 历史命中率 **100%**（537 腿 0% vs 112 腿 100%，零交叠）⇒ **且现盘 gen `#73` 仍复现 ⇒ 缺陷是活的**。
- ⚠️ **它同时证伪了本行旧方子里"回退 `ShowWindow`"的因果读法**（`C0` 显示 `#50` 前"窗口可见"在场却仍不触发）⇒ 该读法**作废**；修法另立 `TASK-0747`（**`F-A` 最小解**：给 shell32 面**补一处**导出 `SHAppBarMessage` —— `ABM_GETTASKBARPOS` 填本移植的工作区事实（`wpf_x11_workarea`），其余返 0 ＋ `E_NOTIMPL`，注释如实写"工作区**近似**"；**否决 `F-C` 回退 `#50`**：`C0` 证明那不是最小解，且会连带退回 `D-G83` 声明线）。
- **对"静默/0 字节/启动期零异常"类判据的影响（重要）**：含 `after-map` 的件**每次启动必被打红**（+242 B；历史 **114/114**）⇒ 这类判据**必须有剔除集或在册声明**（与 `#68` 的 `SILENTHIT` 口径/剔除集直接相关）；`WmGetMinMaxInfo` 末句 `Marshal.StructureToPtr` **每次启动都被异常截断**；但 shim 侧"应用未声明上限"那条读数与异常**无关**（A0／A1 的 diag 逐字节相同）。
- **如实边界（`NOINFO=5`）**：`#50` 四导出里**哪一个**仍未钉（要改 `$R`／重建 ⇒ 该车道不许）｜19 个尺寸变化函数行为面未测｜有 WM 装置未跑｜两件不在盘上（`943d2fcc`／`ce37e262`）｜长跑里异常是否累积未测。另：W159A 的 `strings` 仪器**自判无射程**（`HARD-FAIL`／`STRINGS_POLARITY_HITS=0`）⇒ 只采其修正后 v2 读数；其余关键原始件**由本车道用其自己的原始日志重新现算**（未照抄结论）。
...
- **实例 ③（`2026-09-26` 波 `#75`，车道 W173A 自捉并修，主控落册）＝ 收尾 `trap` 被并进 **`--selftest` 分支** ⇒ **生产路径漏接**（判据"看起来装了、其实没装"）**：它给 `build/MilBridge/tools/frame-presence-check.sh` 加"只收自起 `Xvfb`"的收尾时，把新 `trap` 并进了 **`--selftest` 分支**已有的那条 `EXIT` trap ⇒ **生产路径没有收尾** ⇒ 同波新牙 `X-CENSUS` 在生产门禁里照样红：`X_CENSUS=FAIL leaks=1`（`X_CENSUS_LEAK pid=3329426 display=:97`，`ppid=1`，`lstart=11:51:25` 落在该趟窗口内）。修法＝在**生产分支**（`Xvfb "$DISPLAY_NUM" … &` 之前）**另安一条** `EXIT` trap ⇒ 复核 `X-CENSUS=PASS leaks=0`（`gate1`／`gate2`／`pre` **三趟都是**），且判词成对未变（`SELFTEST=PASS pass=7 fail=0`）。**教训（并入本条口径）**：**凡"装置已装好"的断言，必须由**生产路径的读数**兜住**；只在排练模式里被调，等于没装 —— 这正是本条（判据输入取自瞬时/非生产状态）的又一实例，而且它**由同波新牙当场抓到**。
- **实例 ④（`2026-09-26` 波 `#76` 窗口内，车道 W180A 自伤并自报，主控落册）＝ 落地脚本把"派生路径"绑在**参数解析之前** ⇒ "拷贝进新树、编辑打到旧树"**：`landing.sh` 在**脚本顶层**用**默认** `R` 现推 `VA/CW/FPMS` 三条派生路径，而 `R` 在**参数解析**阶段被 `--repo <沙箱>` 改掉 ⇒ 沙箱排练时**真 `$R` 被写**（`verify-all.sh` `b30c685909832f38→7b339c0c9c229c0c`、`build/close-wave.sh` `d76b0f6668d83f13→5835a01d7d162268`；窗口 `16:24:12–16:25:14`；两件 `%h==1` ⇒ **未写穿**，当场逐件还原并逐位复核）。**危害被放大**：该窗口内**正在跑**的 `#76` chain#2 整趟读数被**连带作废**（`close-wave.sh` **正是当时在跑的脚本** ⇒ bash 对脚本**惰性续读**的风险 ＋ `verify-all.sh` 又在 `fp_inputs` 覆盖面内 ⇒ 覆盖件被改即覆盖读数失真）。**修法（已落）**：新增 `bind_paths()` —— 派生路径**一律在参数解析之后**由**同一个 `R`** 现推，并加三条 `case "$X" in "$R"/*)` **逃逸断言**；**加固时又补掉一个真洞**：只有**字面前缀**闸**抓不到符号链接越界**（`$R/<件>` 若是指向仓外的符号链接，字面前缀照样通过）⇒ 再加 `readlink -f` **解析后**前缀闸。**两极化证据**：**E1（正极）** 异树 `--repo staging/escape-e1` ⇒ `W77_LAND=APPLIED` 且 `SOURCE_TREE_WRITES src_before=0 src_after=0`（**源树零写入**）；**E2（反极）** 把 `$E2/verify-all.sh` 做成**指向真 `$R`** 的符号链接 ⇒ `rc=4 W77_LAND=STOP reason=bind-escape(resolved)`、**写入件数 0**（拒跑发生在**任何写入之前**）。⇒ 已升为**项目级纪律 47**。
...
- **实例 ②（`2026-09-26` 波 `#75`，车道 W173A 现取，主控落册）＝ 门禁步自己起的显示位不收 ⇒ **每趟**复现的孤儿**：`build/MilBridge/tools/frame-presence-check.sh:131` `DISPLAY_NUM="${WPTD_DISPLAY:-:97}"` ＋ `:134` `Xvfb "$DISPLAY_NUM" … &` —— 该件静态审计＝**起 `Xvfb` 1 次 ／ `trap` 0 条 ／ 自持 PID 0** ⇒ 每趟门禁都留一个 `:97` 孤儿（本波两次现场：`pid 3128379`（旧）与 `pid 3329426`，均 `ppid=1`、`lstart` 落在该趟窗口内）。**修法（本波已落）**：起前 `XVFB_OWN_PID=""`／起后 `=$!`／`EXIT` 时**只按该 PID** 收（**并进它既有那条 `EXIT` trap**，不新安第二条以免吃掉 `rm -rf "$T"`）＋ 生产分支另安一条（见 `D-G130` 实例 ③）⇒ 复核 `X-CENSUS=PASS leaks=0`（三趟）。
- **同族另一件（本波同趟修）**：`build/MilBridge/tools/geom-revert-beat-check.sh:158` `Xvfb :227 … &`（只在 `--live-selftest` 走、**不在门禁**）⇒ 同样补"只收自起 PID"的 `EXIT` trap。**其余起 Xvfb 的件已合规**（`tabgap-display-polarity.sh`／`tab-gap-check.sh`／`t1c-census.sh` 都有 `trap` ＋ 自持 PID）。
### 🆕 `D-G140`（**判据装置缺陷 · 全量扫描结论过期 ⇒ 假"无先例"**）：**对"全量语料"的断言（无先例／全绿／零反例）**有失效期** —— 语料一旦增长，旧结论**自动**失效，但断言文本上**没有任何时效标记** ⇒ 后手会拿它当**现行判据**
- **现场（`2026-09-26` 波 `#74` 落仓，主控落册）**：收紧 `NA` 判据（`TASK-0734`）时，曾以"**两臂扫描 49 件 `docs/WAVE*-PREREGISTRATION.md`，无一件 `NA→FAIL`**"作为"**不必动历史件**"的依据；该扫描时刻 `2026-09-25 11:4x`，而**反例是当晚 21:00 之后才出生的** `docs/WAVE72-PREREGISTRATION.md`（其 `:20` 是**被反引号包起来的整条机读行**）⇒ 该件在新判据下被误判成"仅提及" ⇒ `NA→FAIL`，**`verify-all` 第 `[34]` 步当场红、本波冻不了**。⇒ 不是"当初扫错了"，而是**"无先例"这句话会过期**。
- **为什么是缺陷**：① 全量断言被写进报告/预登记后，**读者会当它是当下的性质**；② 语料是**开放集合**（每波都新增 `WAVE*-PREREGISTRATION.md`）⇒ 任何"对全量"的结论**天然是时点读数**；③ 一旦据此做决定（"不必动历史件"），**决定的前提**就在无人察觉时蒸发。
- **修法（口径 ＋ 机制）**：**口径句（逐字，采纳）**：**"凡对**全量语料**的断言（无先例／全绿／零反例），必须写明**语料清单 ＋ 时刻**，并**优先做成接线牙**（每次运行自动重扫）；一次性扫描的结论**只能当当时的读数**，不得当现行判据。"** —— 机制上：把这类检查**接进 `verify-all`**（本例中 `[34]` 正是这样：**接线牙自动重扫 ⇒ 抓到了新增反例**，这正是本缺陷的**正面实例**）。
- **同族但判词不同（不许合并）**：`D-G130`（输入取自瞬时状态）｜`D-G132`（声明没有机读读者）｜`D-G139`（长跑自起件不收 ⇒ 累积）｜本条讲**"对全量"的断言自身会过期**。

### 🆕 `D-G141`（**判据/证据缺陷 · 正则方言误用 ⇒ 恒不中的假证据**）：**写进判据/证据里的正则用了**另一种方言**（最典型：`grep` 不加 `-E` 而模式里写 `A|B|C` ⇒ `|` 被当**字面量**）⇒ 该命令**恒不中**，而读数被当成"**不存在**"的证据**
- **现场（`2026-09-26` 主控在 `TASK-0721` 行文里留档，车道 W175A 现取推翻，主控落册）**：`docs/ROUTES.md` 该行曾写"现场 `grep -rn 'FlowDocument|RichTextBox|TASK-0007' verify-all.sh build/integration-wave.sh` = **0 命中**"，据此判定"**守卫缺失**"。实情：**无 `-E`** ⇒ `|` 当字面量 ⇒ **恒不中**；用 `\|` 现取只中 **1 行**，而那 1 行正是 `verify-all.sh:1090` 把这条坏命令**当证据抄下来的自我引用注释**。而实体**早已存在并接线**（`verify-all.sh:1119 run_step "PTS-PAGES" … --legs`，实跑 `PTS_GUARD=PASS legs=2/2`、`--selftest 12/12`，由波 `#67` 落仓）。⇒ 一条**恒不中的命令**把一个**已办**的事结论成"**缺失**"，并据此排了一条假的活。
- **为什么是缺陷**：与"假绿/假红"不同，这是**假"不存在"** —— 而"**0 命中**"这种读数**天然像结论**（很少有人会去问"这条正则真的能命中吗"）。它同时说明：**证据命令本身也需要被判据守护**（否则它会以"形状完好的读数"混过一切检查）。
- **修法**：**口径句（逐字，采纳）**：**"凡写进判据/证据里的 `grep`/正则命令，必须在**同一趟**给出它的**正极性示例**（喂一个已知命中 ⇒ **必须命中**）；只报『0 命中』而无正极性示例的读数，**不得**作为『不存在』的证据。"** 机制上优先把它并进既有的 `shell-quote-trap-check.sh`／`pipefail-sigpipe-check.sh` 家族（对仓内判据件里出现的 `grep` 模式做方言与**自命中**检查）。
- **同族但判词不同（不许合并）**：`D-G140`（对全量语料的断言会过期）｜`D-G132`（声明没有机读读者）｜`D-G143`（解析失败被静默中性化）｜本条讲**正则方言误用 ⇒ 恒不中的假证据**。

### 🆕 `D-G142`（**判据装置缺陷 · 判据口径文本单点存在**）：**一条判据的**语义口径**（"该判什么、按什么方向判"）**只**存在于一处**历史声明/注释**里，判据件自身**不含**它** ⇒ 那处被改一个字或删掉，判据**照旧跑**、读数**照旧绿**，但**没有人能复算它在判什么**
- **现场（`2026-09-26` 波 `#76` 预备，车道 W175A 现取，主控落册）**：`TASK-0721` 的**判据反转文本**（`洋红=0` 这一方向口径）在仓内**只**存在于 `verify-all.sh:70` 的**一条历史 `DECL` 行**里，而**判据件 `build/MilBridge/tools/pts-pages-guard.sh` 内没有它** ⇒ **单点脆弱**：改那行（或它被压缩/重排）⇒ 判据仍在跑，但**"它判的是哪一列、哪个方向"**就只剩代码可读（同族：`D-G122` 的止损无守卫）。
- **为什么是缺陷**：与 `D-G132`（声明没有机读读者）**方向相反、危害同族** —— 那条是"声明没人读"，本条是"**口径只活在注释里**"。二者合起来说明：**判据的语义必须与判据的代码同址**：判据件里要能读到"我在判什么、按哪个方向判、分母是什么"。
- **修法（`TASK-0741`）**：把口径文本**钉进判据件自身**（件头 ＋ 判词行携带方向标记），并做**两极化**：从件里**删掉**该口径句 ⇒ **必红**（或至少 `NOINFO`，不许静默绿）；在位 ⇒ 绿。**另**：凡"只在历史 `DECL`/注释里"的口径，一律按本条登记，并在同一趟**搬回判据件**。
- **口径句（逐字，采纳）**：**"凡判据，其**口径（判什么、哪个方向、分母给谁）**必须与**判据代码同址**；只在历史声明/注释里存在的口径，视为**未落地**（不许当现行判据）。"**
- **同族但判词不同（不许合并）**：`D-G132`（声明没有读者）｜`D-G108`（声明指向未提交的件）｜`D-G115`（判词缺方向）｜本条讲**口径文本与判据件分居两处**。

### 🆕 `D-G143`（**判据装置缺陷 · 解析失败静默返回中性值 ＋ 继承环境不校验 ⇒ 假负**）：**取数器的解析函数在"格式不符"时**静默返回一个中性/零值**（而不是响亮失败），并且它吃**继承来的环境变量**、**从不校验** ⇒ 该腿的谓词**恒零**、正极性**永远不成立**，于是整条路被读成 `NOINFO`
- **现场（`2026-09-26` 车道 W178A 换路径取证 `TASK-0111`，主控落册）**：`~/w157a/bin/w67-evt.py:99-101` 的 `wh()` 在字符串不匹配 `^\d+x\d+$` 时**静默返回 `(0,0)`** ⇒ `EVT_PUSH_BACK` 谓词**恒零**；同时 `w67-leg.sh:144` `SCREEN="${SCREEN:-1280x1024}"` **只在 env 未设时**才给默认值 ⇒ **继承值全盘接管且从不校验**。后果：归档 `p67a-L1/L2` 记 `EVT_PUSH_BACK=0`，而**同件同参重算 ⇒ `=1`（+740.013 ms）**；**逐字节证**：以 `SCREEN=1280x1024x24` 重算 ⇒ 与归档 `EVT_*` 块 **`diff` IDENTICAL**；全仓 `grep 'EVT_PUSH_BACK=1'` ⇒ **零读数**（只在源码/判据文字里）。⇒ `criteria-w67.md §4.2` 要求的"正极性"因**假负**被判"不成立"，`#67` 那条"新路"因此 `NOINFO` —— **不是世界的问题，是仪器的问题**。**该车道当场复现同一成因**（本批 `SCREEN=0` 被继承）并证明**响亮失败**的形态：守卫逐腿 `FAIL rc=8`（**不是静默 0**）。
- **为什么是缺陷**：解析器的"解析不出"是一个**信息**，返回中性值把信息**抹掉**；再叠上"继承环境优先且不校验"，判据就**在错误的参数上恒零**，而**报告层面只看到 `NOINFO`**（与 `D-G119 ㉖`／`D-G130` 同族，但危害方向是**假负**：**把真发现掐死**，比假绿更难察觉 —— 假绿有人怀疑，假负被当成"世界如此"）。
- **修法（登记为 `TASK-0742`）**：① **解析器：解析不出 ⇒ 响亮失败**（空边一律 `NOINFO`/非零，**禁止**返回 `0`/`(0,0)` 这类中性值）；② **凡"与上一代/归档比"的取数器，必须在同一趟打印它用的**几何/口径参数**（`SCREEN`/`BASE` 等**逐字**）及其**解析结果**；解析不出即失败**；③ 继承的环境变量**必须校验**（白名单格式），不合格即失败；④ 两极化：喂一个格式不符的 `SCREEN` ⇒ **必红/必 `NOINFO`**（**不许**静默零）；喂合法值 ⇒ 绿。
- **口径句（逐字，采纳）**：**"凡解析器，解析不出必须**响亮失败**；凡吃**继承环境**的取数器，必须**校验**它并**逐字打印**；凡『与上一代/归档比』的读数，必须在同一趟打印**参数与解析结果**。"**
- **同族但判词不同（不许合并）**：`D-G119 ㉖`（域选错）｜`D-G130`（输入取自瞬时状态）｜`D-G131`（"取最新"靠位置）｜本条讲**解析失败被静默中性化 ⇒ 假负**。

### 🆕 `D-G144`（**判据装置缺陷 · 符号级 hook 对"经句柄解析的 P/Invoke"恒瞎 ⇒ 假"没调用"**）：**托管层的 P/Invoke 通过 `NativeLibrary` 句柄解析符号**，于是 `LD_PRELOAD` 那种**符号级 hook 永远看不到这次调用** ⇒ 台账上"**没调用**"，而 wire 上**明明有这条请求**
- **现场（`2026-09-26` 车道 W179A 协议层归因 `TASK-0743`，主控落册）**：同一腿上，(a) 符号级 hook 台账**两腿都没有** `CALL XResizeWindow`；而 (b) **协议级**台账**有** `ConfigureWindow(mask=0xc, 1280x1024)`（应用自己的 X 连接 `fd=134`，`getpeername` 断言 `.X11-unix/`，链 `_XSend ← _XReply ← XSync ← wpfgfx_cor3.so+0x131b36/+0x1339f5/+0x16fa4a`），服务器 4.4 ms 后给 `ConfigureNotify synthetic NO`、末态几何逐数相同。**根因**：托管层 X11 P/Invoke 走 **`NativeLibrary` 句柄解析**（`src/WpfGfx.Linux/**/X11Native.cs:11/27/49/291`）⇒ `LD_PRELOAD` 的符号级 hook **对这条路径结构性地失效**。
- **为什么是缺陷（危害方向＝假负）**：这类读数会让人得出"**应用没有调用它**"的结论，从而把因果**错派给别人**（本例真凶是应用自己，而符号级台账会指向 WM／server）；而"**没调用**"这种结论**极易被当成事实**（没人会去怀疑一个 hook 的可见性）。与 `D-G143` 同族（都是**读数缺失被当中性事实**），但成因不同：那条是**解析器**，这条是**观测手段的可见性边界**。
- **修法（登记为 `TASK-0744`）**：凡判"**谁调用了什么**"，必须用**协议级/wire 级**证据（并在读数里打印**连接的 socket 身份**：fd 与 `getpeername`，以区分"应用的连接"与"WM 的连接"）；**符号级 hook 的"没调用"不构成证据**，只能作为旁证；牙要能对"哪个连接写了这条请求"做**归因**并与腿的红/绿做**关联**（本例现算：带该请求的 **27/27 全红**，54 条绿腿 **0 条**有它；`W-OLD` 世界内 **24/24 vs 0/23**，零例外）。
- **口径句（逐字，采纳）**：**"凡判『谁调用了什么』，必须用**协议级/wire 级**证据并打印**连接的 socket 身份**；**符号级 hook 的『没调用』不构成证据**。"**
- **同族但判词不同（不许合并）**：`D-G143`（解析失败被静默中性化）｜`D-G119 ㉖`（域选错）｜`D-G130`（输入取自瞬时状态）｜本条讲**观测手段对某条调用路径结构性不可见**。

### 🆕 `D-G145`（**判据装置缺陷 · 记录件缺机读形态 ⇒ 下一代核不动该字段**）：**记录模板把某个被 `prev_*` 抽取的字段写成**散文/位移句**，于是下一代冻结时**该键的机读形态命中 0** ⇒ `PREVCHECK=REFUSE HITS!=1`（**拒冻**）或被降级成"本代不核它"
- **现场（`2026-09-26` 波 `#74` 冻结，车道 W172A 停手报主控，主控裁定）**：`w27-freeze.py` 的 `_PREV_SRC['prev_infp']` 用机读正则 ``` `inputs_fp` = `<64hex>` ``` 从上一代块抽取；**`#73` 那一块里这个形态命中 0 次**（有界块 46 行内，值本身以**散文/位移句**形式出现两次：`⑤ …；\`inputs_fp\` \`999791b4…\` → \`17f0abdb…\``）。对照 `#72` 块**有**该形态（`:① … 本波整波自印 \`inputs_fp\` = \`999791b4…\``）⇒ 命中 1 ⇒ `#73` 当时能冻。⇒ **`#73` 的记录模板把这一格写成了位移句**（值 `17f0abdb…` **完全正确**）。**处置**：冻结器**拒冻**（`rc=2`）∧ **基线件零字节改动** ∧ **不写 `DONE`** ∧ `CURRENT-STATE` 仍 `gen=#73` ⇒ 守卫行为正确（它拒的是"**核不动**"，不是"值不对"）。
- **为什么是缺陷**：`prev_*` 家族的守卫**只在"记录件带机读形态"时才成立** —— 形态缺失把守卫**降级**成"本代不核该键"或"拒冻"，而**模板的写法是每代各自手写的**（本次就是模板漂移）。它同时说明：**记录件不只是给人读的叙述，它还是下一代的判据输入**（模板的任何"散文化"都在**削弱下一代**）。
- **修法（登记为 `TASK-0745`）**：**冻结器在写记录前自检"本代记录是否带齐下一代需要的全部机读形态"**（逐键对 `_PREV_SRC` 家族求值，**任一键命中 0 ⇒ 拒冻**），并把自检读数打进冻结报告 ⇒ 把"每代模板手工记着写"换成**机器保证前向兼容**。**本代临时处置（主控裁定）**：`GENS['#74']` 撤掉 `prev_infp` 的**块内形态**来源，但**必须**同趟打**替代权威来源**断言 `PREVCHECK_SUBST key=prev_infp src=GENS[#73].infp … eq=yes`（不等 ⇒ 照样拒冻），并如实写明"主来源不可用、替代来源已核"（**不是**"少核一个"）。
- **口径句（逐字，采纳）**：**"凡被 `prev_*` 抽取的字段，记录件里必须逐字带该字段的**机读形态**；散文/位移句**不算** —— 缺形态会让下一代**拒冻**或被迫降级。"**
- **同族但判词不同（不许合并）**：`D-G132`（声明没有机读读者）｜`D-G140`（全量断言会过期）｜`D-G143`（解析失败被静默中性化）｜本条讲**记录件缺形态 ⇒ 下一代守卫核不动**。

### 🆕 `D-G134`（**判据装置缺陷 · `NA`（"本波不适用"）可被**提及**触发 ⇒ 假 N/A**）：**判据只看"判据节里出现过那句话"，于是**提到**与**声明**同权** ⇒ 四要件**一件没写**也能拿到 `NA`（＝获准跳过回归判定四要件）
- **现场（`2026-09-25` 车道 W166A 顶出、**主控亲手复现**；`2026-09-26` 波 `#74` 修法落仓并复核）**：`build/MilBridge/tools/prereg-four-requirements-check.sh:170` 的 `[[ "$sec" =~ 本波.*不做任何回归判定 ]]` 只看**判据节文本**。主控沙箱两臂只差一句：**提及档**（判据节里只写"本节只是讨论了「本波不做任何回归判定」"，四要件一件没写）⇒ `PREREG4=NA`／`rc=0`／`requirements=N/A`；**删掉该句** ⇒ `PREREG4=FAIL`／`missing=8`。⇒ **门被散文打开**。
- **为什么是缺陷**：`NA` 正是"**跳过回归判定四要件**"的那道门 ⇒ 这是**假绿通道**（比单条判据失灵更重：它让整个要件集**合法地**不适用）。
- **修法（波 `#74` 落仓，`a40aac9031304a8f → 56f23e0704232675`）**：`NA` 改成**两形态**（都**只在判据节内**）—— ① **行首锚定声明句**（剥装饰后须以 `本波` 起头且同行含那句话）② **机读行** `PREREG-NO-REGRESSION-DECISION:` 值**非空且非否定令牌**。落仓途中另补 `w74b`：**成对行内代码定界符**也算声明形态（`#72` 的记录件正是被反引号包着的机读行 ⇒ 收紧后曾被误判成"仅提及"）；**只一端反引号 ⇒ 不算**；**包裹内值检查照旧**。**全量批次现读**：`BATCH_RC=0 files=55 pass=1 fail=0 na=16 out_of_scope=38`，逐件与基件臂对账 `NA→FAIL N=0`；`--selftest 18 → 31`（既有例一字未动）。
- **口径句（逐字，采纳）**：**"凡『不适用/跳过』类的门，判据必须绑到**声明的形式**（锚定句或机读行），**不得**绑到『文本里出现过某句话』。"**
- **同族但判词不同（不许合并）**：`D-G119 ㉔`（域选错 ⇒ 全文 `grep` 只作旁证）｜`D-G132`（声明没有机读读者）｜`D-G135`（`NA` 软形态的残余洞）｜本条讲**门被"提及"打开**。

### 🆕 `D-G146`（**判据装置缺陷 · 计数印的是**源表大小**而不是**实核集** ⇒ 形状完好的假读数**）：**判词行里的 `keys=N` 取的是"来源表长度"**，而本代实际核过的键**可能更少**（某个键因**上一代记录件缺机读形态**被跳过）⇒ **`keys=7` 看起来像"7 键都过了主来源"，实际是"6 键主来源 ＋ 1 键替代来源"**
- **现场（`2026-09-26` 波 `#74` 冻结，车道 W172A 自己顶出并如实报，主控落册）**：冻结器生产路径打 `PREVCHECK=PASS gen=#74 keys=7 base=f747350edf97a74a`，而 `keys` 的实现是 `len(_PREV_SRC)`（**来源表大小**）；同一段自述又诚实写着 `⚠️ GENS[#74] 没有 prev_infp ⇒ 本代不核它（如实记录，不冒充已核）`。⇒ **只看 `keys=7` 会以为 7 键都过了主来源** —— 而真相是 6 键主来源 ＋ 1 键（`prev_infp`）用**声明过的替代来源**（`GENS['#73'].infp`）核过。**这与本仓最忌讳的那类形态同族**：**形状完好、字段齐全、但读出来的意思比实况强**。
- **为什么是缺陷**：① 它对**读过它的人**说谎（且是最容易被引用的那种短读数）；② 它**掩盖了**"某键降级"这一事实（降级本身是**已知且可接受的**，但必须**可见**）；③ 它与 `D-G145` 形成闭环病灶：**上一代缺形态 ⇒ 本代跳过 ⇒ 本代印"满数"** ⇒ 缺口在两代之间**看不见**。
- **修法（`TASK-0746`）**：`keys=` 必须等于**实核集**大小 —— 跳过某键时该数**必须减一**，或改印 `keys=7(checked=6 skipped=prev_infp)`（推荐后者：既给总数也给实核数）；**并要求**：任何"跳过/降级"必须**同趟**打一条**具名**机读行（`PREVCHECK_SKIP key=… reason=…`）。**两极化**：① **强制跳过**某键（沙箱：把该键的来源从基线块里去掉）⇒ `keys` **必减**（或 `skipped=` 必现该键）；② **不跳过** ⇒ `keys` **等于**来源表大小。
- **口径句（逐字，采纳）**：**"凡计数类读数，必须声明它数的是**什么集合**（源表／实核集／命中集）；把'源表大小'印成 `keys=` 会让读数**比实况强** —— 跳过/降级必须**具名可见**。"**
- **同族但判词不同（不许合并）**：`D-G145`（记录件缺形态 ⇒ 下一代核不动）｜`D-G132`（声明没有机读读者）｜`D-G129`（状态位与报告不符）｜本条讲**计数的集合名不对 ⇒ 读数比实况强**。
