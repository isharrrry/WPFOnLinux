...
- **修法（新 `TASK-0722`，排 `#68`）**：① 把应用**自报插桩行** `[HC-UNHANDLED] #N` 也纳入剔除集；② **两个数都印**（原始字节数 ＋ 剔除后字节数）；③ **跨件代不许混比**（分母按件代分开）；④ 两极化：**校准臂（WC07 触发件）必红**／**现件代必绿**；⑤ 重测趟数按在册口径（`≥131 腿` ⇒ 2.26%／`≥299 腿` ⇒ 1.00%），**先写趟数与功效**。
