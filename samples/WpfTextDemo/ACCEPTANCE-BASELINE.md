# BASELINE-HEADER date=2026-09-25T10:xx+08:00 display=:23x（`verify-all` 头标目标显示号）｜车道 = **W162A-W68PREP**（`#68`：`TASK-0722` 落仓 —— 把 `D-G123` 的口径修法做成**仓内判据端牙**并接进 `verify-all` 第 `[39]` 步；**owner = 本车道**）
#   cpu=4核  run_dir=/home/links-dev/w162a/w68 ｜车道工作目录 `~/w162a/`（`criteria.md`／`land/`＝落仓件与前置闸／`w68/`＝链条与冻结）｜`$R` **不是** git 仓（推送面 = Gitee `WPFOnLinux` 的 `feat-Linux`）
#   ⚠️ 权威件构建配置 = Release（唯一声明 = build/SelfBuiltConfig.props）
#   判据先行：`~/w162a/criteria.md`（`74a5dae9d6d19106`）｜放行件 `~/w129a/dispatch-W68-landing.md`（`84f20f1f89ed2c96`）｜落仓前置闸 `~/w162a/land/apply.sh`（`93d8fef391384cd7`）＋ `land/commit.sh`
#   ⚠️ `docs/WAVE68-PREREGISTRATION.md`（`c48fb294a991c640`）与**落地同趟**产出（标题行含 `#68` 是第 `[11]` 步的硬要求）。
#   ⚠️「本波不做任何回归判定」声明**落在 §2 判据节内**（`#66` 的教训）—— 落地后现跑 `PREREG4=NA rc=0`。
#   ⚠️ **射程边界（如实，不许读成绿）**：本波落的是**判据端** ⇒ `SILENT-HIT-V2` 只证「判据装置还活着 ∧ 不许把不可判当零命中」，
#      **不**证「现件代静默 SEGV 已复现／已清零」；产出端（腿驱动）**仍在车道**（`UNWIRED`，见 FROZEN 段）。

# RE-FROZEN #68 —— ✅ **当前冻结基线** —— 内容 = **`TASK-0722` 落仓**：`D-G123` 的口径修法（判据端入仓，`verify-all` 第 `[39]` 步 `SILENT-HIT-V2`）＋ 四处声明 ＋ `fp_inputs()` 两行。**本波零产品位移**（不改任何产品件；九位里只有环成员 `pf` 变）。
