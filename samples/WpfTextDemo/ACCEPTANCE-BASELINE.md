# BASELINE-HEADER date=2026-09-24T02:xx+08:00 display=:99（verify-all 头标目标显示号；本趟现场判 `X-REUSE=reused`、`X_STATE=available`、`SKIP_GUARD=PASS`）
#   cpu=3核  run_dir=/home/links-dev/w150a（收尾链 ①–⑨）｜门禁 rows = /home/links-dev/w150a/w58-rows.txt（**本趟 6 行**，从门禁装置追加日志里按**终态 config** 过滤得到；全 `result=PASS`）
...
#   车道 = **W150A**（落地 ＋ 收尾链）｜判据 `~/w150a/criteria.md`｜链报告 `~/w150a/report.md`（逐段追加，`APPEND_ONLY`）

# RE-FROZEN #58 —— ✅ **当前冻结基线** —— 内容 = **仪器波**（**零产品改动**）：`TASK-0705`「回归判定四要件」整包落地 —— A 半模板六处插入 ＋ 新牙 ＋ B 半牙本体三缺陷修复
