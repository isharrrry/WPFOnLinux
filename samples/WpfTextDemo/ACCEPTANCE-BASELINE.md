# BASELINE-HEADER date=2026-09-25T2x:xx+08:00 display=:23x（`verify-all` 头标目标显示号）｜车道 = **W170A-W72LAND**（`#72`：「冻结器与抽取器锚在**语义**上」—— `TASK-0730` 让冻结器**自动核每个 `prev_*`** 与基线块逐位一致、不符即**拒冻**且**基线件零字节改动**，并把 `B.pre-freeze.<gen>.bak` 做进冻结器本身；`TASK-0731` 把「找块」抽取器的**出口条件**锚到行首）
#   cpu=4核  run_dir=/home/links-dev/w170a/w72 ｜车道工作目录 `~/w170a/`（`criteria.md`／`sbx/`＝影子沙箱／`logs/`＝链条日志）｜`$R` **不是** git 仓
#   ⚠️ 权威件构建配置 = Release（唯一声明 = build/SelfBuiltConfig.props）
#   ⚠️ 判据先行：`~/w170a/criteria.md`（`a557c1f048c10928`，22:13:13）｜放行件 `~/w129a/dispatch` ＋ 主控 `#72` 四条附加硬条件（出口锚的域要自证／块尾找不到要响亮／四腿进 `--selftest`／冻结器耐久化）
#   ⚠️ `docs/WAVE72-PREREGISTRATION.md` 与**落地同趟**产出（标题行含 `#72` 是第 `[11]` 步 `VERIFYALL-SELF` 的硬要求）。
#   ⚠️「本波不做任何回归判定」**机读行** `PREREG-NO-REGRESSION-DECISION: yes` **落在 §2 判据节内**（纪律 45）。
#   ⚠️ **射程边界（如实，不许读成绿）**：① `prev_*` 自动核**只对"待冻代的上一代块"**生效（现取：51 个块里只有 `#71`／`#69`／`#68`／`#67` 的来源形态 8/8 全可抽）⇒ 更老代会被**响亮拒冻**，**不**降级成静默放行；② `#67` 的**原始现场件未留存** ⇒ 「复现 `#67` 原样」是 `NOINFO`，本波只复现**同类形态**（`C0731a` 用真仓 `#52` 块、`C0731b` 用合成档）；③ 出口锚是一份**形态白名单** ⇒ 已加"两种机制数块头必须相等"的自证，但它**不能证明第三种形态不存在**，只能说"出现了就会被点名"。

# RE-FROZEN #72 —— ✅ **当前冻结基线** —— 内容 = 「**冻结器与抽取器锚在语义上**」落仓（`TASK-0730` ＋ `TASK-0731`）：① 冻结器（**仓外** `~/w21-verify/w27-freeze.py`）新增 `check_prev_values()` —— `prev_pf`／`prev_pc`／`prev_wb`／`prev_wsh`／`prev_dwf` 取基线件**九位行**、`prev_bsfp`／`prev_infp` 取块内 `BRIDGE_SRC_FP`／`inputs_fp` 行，**每个来源命中数须恰为 1**，并与 `BASELINE tier=` 交叉核；不符 ⇒ **拒冻（rc=2）∧ 基线件零字节改动**（核验插在**任何写盘之前**）。同趟把 `B.pre-freeze.<gen>.bak` 做进冻结器（`cp -a` 真拷贝 ＋ 逐字节断言 ＋ `%h==1`）；② `build/MilBridge/tools/column-floor-check.sh` 的 `extract_newest_block()` **出口条件锚行首**（旧写法用**无锚**的「`RE-FROZEN` ＋ 井号」当退出条件 ⇒ 块内正文只要提到该字样就提前截断，把紧随其后的声明行全排除 ⇒ `COLUMN_FLOOR=NOINFO`（**病因错**）），并新增两条**响亮**守卫（块尾找不到 ⇒ `NOINFO reason=block-end-not-found`；出口锚的**域**由两种机制现取自证 ⇒ `NOINFO reason=block-header-form-mismatch`）。
