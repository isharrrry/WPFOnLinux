# BASELINE-HEADER date=2026-09-26T1x:xx+08:00 display=:23x（`verify-all` 头标目标显示号）｜车道 = **W173A-W75LAND**（`#75`：`TASK-0736`「件头自述 vs 接线（做牙）」＋ `TASK-0737`「落地件不许带车道路径（做牙 ＋ 修三处真默认值）」＋ `TASK-0738`「比较域必须区分读数与标签」＋ `TASK-0739`「长跑自起显示位收尾收净 ＋ 普查牙」—— 四件**合波**，主控裁定「四波并为一步」以减串行链数）
#   cpu=4核  run_dir=/home/links-dev/w173a ｜车道工作目录 `~/w173a/w75/`（`criteria.md`／`logs/`＝链条日志／`w75freeze/`＝冻结物）｜`$R` **不是** git 仓
#   ⚠️ 权威件构建配置 = Release（唯一声明 = build/SelfBuiltConfig.props）
#   ⚠️ 判据先行：`~/w173a/w75/criteria.md`（写于**任何 `$R` 写入之前**，四件各自判据 ＋ 判否条件 ＋ 三态 ＋ 输入来源声明 ＋ `NOINFO` 清单）｜放行件 `~/w129a/dispatch-W68-landing.md` ＋ 主控对本波的四次裁定（`0737` 的 (甲)＋(乙)／`0739` 只并 ①②／`0738` 新牙／`skip=3` 声明值 ＋ 冻前销账）
#   ⚠️ `docs/WAVE75-PREREGISTRATION.md` 与**落地同趟**产出（标题行含 `#75` 是第 `[17]` 步 `VERIFYALL-SELF` 的硬要求）。
#   ⚠️「本波不做任何回归判定」**机读行** `PREREG-NO-REGRESSION-DECISION: …` **落在 §2 判据节内**（纪律 45）—— 落地后判 **`PREREG4=NA`（rc=0）** ∧ **`form=machine-line(…)`**；四件全部是**装置/判据卫生**，判的是牙自己，不判产品回归。
#   ⚠️ **射程边界（如实，不许读成绿）**：① `0736` 的牙**只判"自述与现场相反"** —— 件头**没写任何自述**的件（`undeclared=47`）**不判红**（那是一次改几十件件头 = 主控裁定范围，不是本牙射程）；② `0737` 的牙**只判形状**（命中/未命中 ＋ `kind=code|comment|data`），**判不了**"某个默认路径在语义上算不算仓内"；且**只扫代码件**（`*.sh`／`*.py`）—— 证据/台账里的车道路径是**取证内容**、不判也不许改；③ `0738` 的牙只比**同一冻结块内同一 `tier` 的各 `rep`**（跨块比是另一回事：同代 `pf` 会变），且**不产**门禁链的 `GATE_LINES_IDENTICAL`（那是链外脚本的口径）；④ `0739` 的"有主/无主"**只认 `Xvfb` 主人** ⇒ 非 `Xvfb` 的 X server 持有的 socket 只作**见证**、不判红；**修法③（显示位改"租用"、去硬编码 `:97`／`:99`）本波 `NOINFO` ⇒ 另开一波**；⑤ 覆盖面只保护"**改了会被看见**"，**不判**"该收的没收"。

# RE-FROZEN #75 —— ✅ **当前冻结基线** —— 内容 = 「**四件装置/判据卫生**」合波（`TASK-0736` ＋ `TASK-0737` ＋ `TASK-0738` ＋ `TASK-0739`，**仪器波 · 零产品改动**）：① **件头自述 vs 接线**（新牙 `build/MilBridge/tools/selfdescription-wiring-check.sh`：件头含"未接线"而 `^run_step` **命中**该件 ⇒ 必红；**反向**（自述"已接线"而**零命中**）⇒ 也必红；`--selftest` 8/8）＋ 同趟修 `prereg-four-requirements-check.sh` 件头的陈旧自述（**不写死步号**）＋ **接线为一步** `SELFDESC-WIRING`；② **落地件不许出现车道路径**（新牙 `build/MilBridge/tools/lane-path-check.sh`：`kind=code` 必红且**永不许豁免**；`comment`／`data` 走**声明式出处清单** `build/MilBridge/lane-path-provenance.tsv` ＋ **每类件数上限**（现读 > 上限 ⇒ 红）；`--selftest` 16/16）＋ **同趟修三处真默认值**（`hidden-only-step.sh`／`geom-revert-beat-check.sh`／`tests/W81AWindowProbe/run-w81a-legs.sh`）；③ **比较域必须区分"读数"与"标签"**（新牙 `build/MilBridge/tools/rows-identity-check.sh`：只标签差异 ⇒ `PASS` ＋ **必印** `LABEL_ONLY_DIFF fields=…`；读数真差异 ⇒ `FAIL`；语料＝`samples/WpfTextDemo/ACCEPTANCE-BASELINE.md` **最新冻结块现取**；`--selftest` 12/12）；④ **长跑自起显示位收尾收净 ＋ 自查**（`verify-all.sh` 记本趟自起 `Xvfb` PID ＋ `EXIT trap` **按 PID** 收；**复用者／`--no-x` ⇒ 一个都不杀**；新牙 `build/MilBridge/tools/xvfb-census-check.sh` 以 `[0]` 段落的**自含链前基线**比对 `ps` ＋ `/tmp/.X11-unix/`，差异**点名 PID**；"死 socket"与"活泄漏"**分开判**；`--selftest` 12/12）。
