# BASELINE-HEADER date=2026-09-24T11:3x+08:00 display=:99（verify-all 头标目标显示号；本趟现场判 `X-REUSE=reused`、`X_STATE=available`、`SKIP_GUARD=PASS`）
#   cpu=4核  run_dir=/home/links-dev/w153a（收尾链 ①–⑨）｜车道 = **W153A**（`#61`：判据先行 ＋ 落地 ＋ 收尾链的唯一执行者）
...
#   判据 `~/w153a/criteria.md`（**判据先写**：`§0–§7` 写定于取任何"修后"自有读数之前）｜链报告 `~/w153a/report.md`（逐段追加）

# RE-FROZEN #61 —— ✅ **当前冻结基线** —— 内容 = **判据接线/射程波 · 零产品改动**：把三处**能造出假绿/假红的旋钮**接死 —— ① **`D-G113`「假旋钮」⇒ connect**（`--fix=`/`--pre=` **真的**进判据）② **`TASK-0710`** `--leg=` 旁路 ⇒ **judge**（派生语料**真判**、`--leg` 档 `NOT_APPLICABLE` 且**不传染**）③ **`TASK-0711`** 仓内 runner **首写点截断**（`>>` → `>`）。**三件都不加步**（**34 → 34**）。
