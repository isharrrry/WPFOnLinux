# BASELINE-HEADER date=2026-09-26T0x:xx+08:00 display=:23x（`verify-all` 头标目标显示号）｜车道 = **W171A-W73LAND**（`#73`：`TASK-0726`「把**静默 SEGV** 那条腿的**产出端**收进仓 ＋ 收编同形副本」）
#   cpu=4核  run_dir=/home/links-dev/w171a/w73 ｜车道工作目录 `~/w171a/`（`criteria.md`／`logs/`＝链条日志）｜`$R` **不是** git 仓
#   ⚠️ 权威件构建配置 = Release（唯一声明 = build/SelfBuiltConfig.props）
#   ⚠️ 判据先行：`~/w171a/criteria.md`｜放行件 `~/w129a/dispatch-W68-landing.md` ＋ `~/w129a/dispatch-W165A-0726.md` ＋ 主控 `#73` 两条附加条件（剔除集**单源核查**／落 2 件批）
#   ⚠️ `docs/WAVE73-PREREGISTRATION.md` 与**落地同趟**产出（标题行含 `#73` 是第 `[11]` 步 `VERIFYALL-SELF` 的硬要求）。
#   ⚠️「本波不做任何回归判定」**机读行** `PREREG-NO-REGRESSION-DECISION: …` **落在 §2 判据节内**（纪律 45）—— 落地后 `prereg-four-requirements-check.sh` 判 **`PREREG4=NA`（rc=0）**，与 `pass=` **分开计数**。
#   ⚠️ **射程边界（如实，不许读成绿）**：① 产出端**只**把"这一趟怎么被观测"变成机读行，**不判**"现件代静默 SEGV 已清零"；② 现场形态**不跑 gdb** ⇒ 现场可观测的第三支是 `rc139`／`fate`，`term`／`stop-signo11` 只在 `--replay` 形态可判；③ `--app-cmd` 是**极化/自检形态**（应用面被调用方替换）⇒ 其读数**不是** WPF 应用本体的读数；④ **`UNWIRED` 仍在**：产出端**在仓内**但 `verify-all` **没有任何一步调用它**（判据口径限定到代码形状 `grep -cE '^[[:space:]]*run_step .*silenthit' verify-all.sh` = 0）⇒ **不许**把第 `[39]` 步的 `--cases PASS` 读成「现件代已复现／已清零」。

# RE-FROZEN #73 —— ✅ **当前冻结基线** —— 内容 = 「**静默 SEGV 那条腿的产出端收进仓**」落仓（`TASK-0726`）：① 新建 `build/MilBridge/tests/SilentHitProbe/run-silenthit-legs.sh`（仓内**唯一**产出端：起显示、起应用、跑 9 击配方、收装置、把观测变成机读行）＋ 同目录 `silenthit-trim.tsv`（剔除集**唯一来源**）；② `build/close-wave.sh` 的 `fp_inputs()` 白名单 **+2 行**；③ `verify-all.sh` 第 `[42]` 步 `--expect 192 → 194` ＋ 头注释两处声明；④ 车道 **7 份**同形副本标 `DEPRECATED-BY=… (WAVE73)`（`~/w128a/bin/one128.sh` 留作历史真命中的产出者证据，**未动**）。**本波零产品位移**（不改任何产品件；九位里只有环成员 `pf` 变）。
