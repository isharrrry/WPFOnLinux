// ⚠️ 本文件由 src/WpfGfx.Linux.Native/tools/patch-windowsbase-msgflow.py **生成**，不要手改。
//
// 内容 = 上游 `upstream/wpf/src/Microsoft.DotNet.Wpf/src/WindowsBase/System/Windows/Threading/Dispatcher.cs` 逐字复制 + 4 处 T1c **只读插桩**（托管侧出队读数）。
...
*/
        ...
        {
            bool handled = false;
            // ── T1c 只读插桩：**托管侧第一个看见这条 MSG 的地方**（GetMessage 刚交出来）──
            ...
            // ── T1c 只读插桩：preprocess filter（HwndSource.OnPreprocessMessage 就在里面）有没有把它吃掉 ──
