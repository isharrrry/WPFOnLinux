...
| `build/MilBridge/W111A-report.md`（本件，新建） | — | **v1** `a7052a887fc83130`（28,601 B，**= 第九笔**） | 28,601（v1） | 新建（**v2 的值不进本文件**，见 §7 末段：记 `~/w111a/STATUS.md`） |
...
- ⚠️ **本报告本体不在第八笔里**：它作为**第九笔**（`1e06b58b1ee4b5a4a7c7ccad0d8f20764837a522`，本文件单独一笔）推送 —— 因为报告里要写第八笔的 head／`BYTECHECK`（**报告写不出自己所在 commit 的哈希**，写了就永远滞后一笔，与 `#50`"冻后刻九位不可能落在冻结块内"同一条道理）。⚠️ **本报告自身还有一笔**：v1 推上去后本车道现场核出 §0 表里"本件行"两格写的是"见文末末行"而**文末并没有那个值**（自指口径写歪）⇒ **第十笔 = 本报告 v2**（只改这一行 ＋ 本句；**判词与读数一字未动**）⇒ **最终 head 记在 `~/w111a/STATUS.md` 与回件消息里**，本文件内不写它。
