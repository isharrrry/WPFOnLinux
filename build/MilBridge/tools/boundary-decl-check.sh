...
invoked_tokens_in() {  # <file> ⇒ 该文件里**以命令形态被调用**的脚本令牌
  ...
  # ⚠️【波 `#77` 修 · 现场】形态②原先写成 `^[[:space:]]*[\`(]*<path>` —— 允许**行首一个反引号**，
  #   于是**散文续行**（多行注释块里以 `` `build/close-wave.sh` `` 起头的那种行）被当成"直调"
  #   ⇒ 把**只被读、从不被执行的件**（`close-wave.sh`）拉进调用闭包，再从它的**白名单/printf 参数表**
  #   里把**从未被调用的件**（`run-silenthit-legs.sh`）拉进来 ⇒ 令声明
