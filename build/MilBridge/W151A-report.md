...
### §11.10 **判据欠账（本波不修，归下一颗仪器牙）** —— 逐条登记

1. **`prereg-four-requirements-check.sh` 对"不做回归判定的波"要求补四要件**（主控 2026-09-24 裁定登记）。
   **现象**：本波（`#59`）是**仪器接线波**、**不做任何回归判定**，而该牙**机械要求**每份 `>= #58` 的预登记在判据节里出现 `PREREG-REGRESSION-FOUR` 机器声明 ＋ 四要件 ＋ `regression-decision.py` 引用 ＋ `REGRESSION`/`NOINFO` 三态词 —— 于是 `WAVE59` 一开始 **`PREREG4=FAIL missing=7`**。
   **本波的处置（合规但笨）**：补了一节**条件句**（§2.7），明写"本波不做任何回归判定 ⇒ 本条按条件句读、**不被触发**" —— **不假装做过**。结果 `PREREG4=PASS requirements=4/4`、全量 `files=40 pass=2 fail=0 skip=38`。
   **更正确的形态**：该牙应能**识别显式的"本波不做回归判定"声明**（例如一个 `PREREG-REGRESSION-FOUR: n/a` 或 `not-applicable` 的机器值）并把该要求标 **`N/A`（**可见**，与 `PASS` 分开计数）**，而**不是**逼着每一份预登记都抄一节。⇒ 那属**下一颗仪器牙**的判据问题，**本波只登记不修**（改它是改判据件 ⇒ 要另走一次两极化 ＋ 会再动 `fp_inputs`）。
2. **`generation.geom_corpus` 的读者射程**：本波已让牙1 在 `--corpus=` 模式下真读（不等 ⇒ `NOINFO` ＋ 点名），但 **`--leg=` 模式按定义不适用** ⇒ 那条路是**声明过的旁路**（预登记 §5.7 的两条护栏）。若下一趟要彻底关掉，可加 `--require-corpus-anchor` 之类的强制档，或照 `arm-log-sha-check.sh` ＋ 第 `[8]` 步先例加一件**独立外挂读者**。
3. **`APPSYNC` 的 `UNEXPECTED=6[DECL-GAP-EQ=6]`**：本波**未**逐条核它是"本次引入"还是"继承"，但已用 `#58` 的 record 做**形态对照**（`#58` 同为 `UNEXPECTED=6[DECL-GAP-EQ=6]`、`STALE=0`）⇒ 判定为**继承自上一代、非本波引入**；**逐条归因**仍记 **`NOINFO`**，留给下一趟。

*`report.md` 自身 sha16 = `464736410fb744b0`（**口径** = `head -n -2`：去掉本行与末行 `W151A=DONE` 摘要行。本波按派单书要求末行必须是机器摘要，故 sha 行上移一行、口径由 `head -n -1` 改为 `head -n -2`）*
`W151A=DONE wave=#59 landed=6件+语料 repin=REPIN_GENERATION_PASS chain=①-⑨全绿 plan=77556f5e180095a9 criteria=161eafd30a698fe9 land=44310c7d6a943e91 baseline=#59/02f80e388d308c4d/846233B(prev #58 ca6f3091995bd49c/829126) verify_all=51ea0b5623981406->ec29c571be6b19da(1103行/run_step=33) tooth1=9412ec0149111348->ee43a703736489a3(701,selftest19/19,live4/4) tooth2=cd375326b62f7982(未改) prereg4=57de293df5be263d->b436ae561ae1f396(319,selftest12/12) closewave=0cf2ea72bdee142c->053f5820e354c5ed knownred=64b22d35efc1e3be->b3f264bbf43dbd61(497) corpus=234件/聚合0f702ccd8eb3938a… 九位位移=只有pf(c69bc1742cfa7f39->70f5fd87457ca0f3,同尺寸6123520) infp=4c4d99f569a00380…->e7b94e10171b55e1…(coverage_n155->157) gate_x2=WPTD_GATE_PASS_acceptance2/2_判词6/6逐字一致 pre_verify=33通过/0失败/875通过/2跳过(全绿,声明类红[]) post_verify_x2=各33✅/0❌ NOFILE_SWAP=YES push=8d0c0d5..45c6628 head=45c6628b5882a175 BYTECHECK=245/245 mismatch=0 appsync=STALE0/DIVERGENT0 prereg4_glob=files40/pass2/fail0/skip38 pipfail=PASS/undeclared_hit=0 report=464736410fb744b0 R_touched=6件+234语料 heavy=YES(槽内) residual=APPSYNC_DECL-GAP-EQ=6(继承自#58,非本波)`
