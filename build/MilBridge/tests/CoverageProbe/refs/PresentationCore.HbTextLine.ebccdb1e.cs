...
namespace WpfLinux.Shims.PresentationCore
{
    ...
    internal static unsafe class HbShaper
    {
        ...
        /// <summary>
        /// **规则 4（Tab 口径，真机实测见 T1d 报告 §10）**：`\t` 的 advance = **0**。
        ///
        /// 【真值依据】真机 `layout-b34` 的 34 个 Tab 用例（文本 `'a\tb\t\tc\td'`）里，
        ///   无限宽那一行 `w=36.3500` = **四个字母之和**（`\t` 一个都不算）；`w=20` 那行以 `\t` 结尾
        ///   而 `w == witw`。⇒ `\t` 对 Width / WITW 都不贡献。
        ///
        /// 【顺带把 `.notdef` 豆腐块去掉】字体里通常没有 U+0009 的字形（`hb_font_get_nominal_glyph`
        ///   返回 0 ⇒ `.notdef`）。若原样画出去，会在**零步进**的位置叠一个豆腐块。
        ///   真机的 tab 不产生墨迹 ⇒ 这里换成该字体的 **space 字形**（无墨迹、无步进）；
        ///   取不到 space 字形就只把步进清零（如实保留 .notdef，不假装）。
        /// </summary>
        private static void ZeroTabAdvances(HbShapedRun r, string text, IntPtr font)
        {
            if (text == null || text.IndexOf('\t') < 0) return;
            uint spaceGlyph = 0;
            bool haveSpace = hb_font_get_nominal_glyph(font, 0x20u, out spaceGlyph) != 0 && spaceGlyph != 0;
            for (int i = 0; i < r.Glyphs.Length; ++i)
            {
                uint cl = r.Clusters[i];
                if (cl >= (uint)text.Length || text[(int)cl] != '\t') continue;
                r.AdvancesPx[i] = 0;
                r.OffsetsXPx[i] = 0;
                if (haveSpace) r.Glyphs[i] = (ushort)spaceGlyph;
            }
        }
    ...
    internal static unsafe class HbFaceCache
    {
        ...
        private static Entry Load(string path, int faceIndex)
        {
            ...
            try
            {
                ...
                fixed (byte* p = pathZ) blob = HbShaper.hb_blob_create_from_file(p);
            ...
            {
                if (blob != IntPtr.Zero) HbShaper.hb_blob_destroy(blob);   // face 自己持有一份引用
            }
        }
        ...
        internal static int FaceCountOf(string path)
        {
            ...
            lock (s_gate)
            {
                ...
                finally { if (blob != IntPtr.Zero) HbShaper.hb_blob_destroy(blob); }
                s_faceCounts[path] = n;
                return n;
            }
        }
        ...
        private static int CountGlyphs(Entry e)
        {
            ...
            finally { HbShaper.hb_blob_destroy(blob); }
        }
    ...
    internal static class HbFontCandidates
    {
        ...
        internal static void EnsureScan()
        {
            lock (s_gate)
            {
                ...
                foreach (string f in files)
                {
                    ...
                    for (int i = 0; i < n; ++i)
                    {
                        HbFaceCache.Entry e = HbFaceCache.Get(f, i);
                        if (e == null) { ++failed; continue; }
                        HbFaceCache.LoadMeta(e);
                        list.Add(new Cand
                        {
                            Path = f, FaceIndex = i, Family = e.Family,
                            Weight = e.Weight, Width = e.Width, Slant = e.Slant,
                        });
                    }
                }
                s_all = list;
                                            ...
                                            out Cand hit, out int probed)
        {
            ...
            foreach (Cand c in s_all)
            {
                ...
                ++probed;
                HbShaper.CoversFace(c.Path, c.FaceIndex, cp);
    ...
    internal static class HbBreakEngine
    {
        ...
        /// 其余空白（空格、全角空格…）照旧计入。
        /// </summary>
        internal static bool IsTrailingWhitespace(char c) => char.IsWhiteSpace(c) && c != '\t';
    ...
    internal sealed class HbTextLine : TextLine
    {
                           ...
                           double paragraphWidth = 0, TextRunProperties[] runProps = null)
        {
            ...
            if (shaped.Glyphs.Length > 0)
            {
                if (shaped.Chunks == null || shaped.Chunks.Count <= 1)
                {
                    ...
                    _glyphRun = BuildGlyphRun(shaped, glyphTypeface, pixelsPerDip, baseline, 0, shaped.Text);
                }
                else
                {
        ...
        internal static HbTextLine FormatLine(
            ...
            TextRunProperties[] runProps = null)
        {
        ...
        public override void Draw(DrawingContext drawingContext, Point origin, InvertAxes inversion)
        {
            ...
            try
            {
                DrawCore(drawingContext, origin, inversion);
            }
            finally
            {
        ...
        private void DrawCore(DrawingContext drawingContext, Point origin, InvertAxes inversion)
        {
            ...
            TextRunProperties[] runProps = null)   // 逐 run 的 props（前景色跟着源 run 走；null ⇒ 与今天逐位相同）
        {
            ...
            foreach (HbLineRange r in ranges)
            {
                ...
                HbTextLine line = HbTextLine.FormatLine(
                    ...
                    alwaysCollapsible, hasModifierScope, lineHeight, plan, segmentFaces, paragraphWidthDip, runProps);
                lines.Add(line);
