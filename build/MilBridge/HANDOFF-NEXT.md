...
## §6 未结清的账（主控侧）
- **未推的本地领先件（"我"侧）**：`docs/ROUTES.md`／`samples/WpfFeatureProbe/KNOWN-DEFECTS.md`／`build/MilBridge/tools/defect-registry-declared.tsv`／`build/MilBridge/HANDOFF-NEXT.md` —— 它们**都不在 `fp_inputs()`**（改它们不动指纹），但**必须在下一波推送时逐径带上**，且**逐件核到远端**（纪律 29）。
