// ⚠️ 本文件由 src/WpfGfx.Linux.Native/tools/patch-presentationcore-inputtrace.py **生成**，不要手改。
//
// 内容 = 上游 `upstream/wpf/src/Microsoft.DotNet.Wpf/src/PresentationCore/System/Windows/InterOp/HwndSource.cs` 逐字复制 + 9 处 T1c **只读插桩**（输入链：WM_CHAR → TextInput）。
        ...
        private object OnPreprocessMessage(object param)
        {
            ...
            case WindowMessage.WM_SYSDEADCHAR:
                ...
                if(!_eatCharMessages)
                {
                    ...
                    WpfLinuxInputTrace.PreprocessCharStep("TranslateChar", msgdata.handled);

                    if (!msgdata.handled)
                    {
                        msgdata.handled = ((IKeyboardInputSink)this).OnMnemonic(ref msgdata.msg, modifierKeys);
                        WpfLinuxInputTrace.PreprocessCharStep("OnMnemonic", msgdata.handled);
                    }
                    ...
                    if (!msgdata.handled)
                    {
                        ...
                        WpfLinuxInputTrace.PreprocessCharStep("ProcessTextInputAction", msgdata.handled);
                    }
                }
...
//             VerifyAccess();
        ...
        {
            _eatCharMessages = false;
            WpfLinuxInputTrace.RestoreCharMessagesCalled();
            return null;
        }
