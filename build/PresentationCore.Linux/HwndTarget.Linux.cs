// ⚠️ 本文件由 src/WpfGfx.Linux.Native/tools/patch-presentationcore-hwndtarget-trace.py **生成**，不要手改。
//
// 内容 = 上游 `src/Microsoft.DotNet.Wpf/src/PresentationCore/System/Windows/InterOp/HwndTarget.cs` 逐字复制 + 5 处只读插桩（`D-G54`：弹窗目标为何没走到 `SetRoot`）。
...
namespace System.Windows.Interop
{
    ...
    public class HwndTarget : CompositionTarget
    {
        ...
        {
            WpfLinuxHtTrace.Log("CreateUCEResources 进入：channel=" + WpfLinuxHtTrace.Id(channel) + " oob=" + WpfLinuxHtTrace.Id(outOfBandChannel));
            ...
            WpfLinuxHtTrace.Log("建目标资源后：resourceCreated=" + resourceCreated + " target(OOB)=" + WpfLinuxHtTrace.H(_compositionTarget.GetHandle(outOfBandChannel)));
            Debug.Assert(resourceCreated);
            _compositionTarget.DuplicateHandle(outOfBandChannel, channel);
            WpfLinuxHtTrace.Log("复制句柄后：target(主通道)=" + WpfLinuxHtTrace.H(_compositionTarget.GetHandle(channel)) + " IsOnChannel(main)=" + _compositionTarget.IsOnChannel(channel));
            ...
            WpfLinuxHtTrace.Log("SetRoot 之前：target=" + WpfLinuxHtTrace.H(_compositionTarget.GetHandle(channel)) + " contentRoot=" + (_contentRoot.IsOnChannel(channel) ? WpfLinuxHtTrace.H(_contentRoot.GetHandle(channel)) : "not-on-channel"));
