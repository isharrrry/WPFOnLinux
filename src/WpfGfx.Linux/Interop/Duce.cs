...
//
// DUCE 资源类型与句柄。
// 来源：Common/Graphics/wgx_core_types.cs:783 (ResourceType)
//      Common/Graphics/exports.cs:968       (ResourceHandle)
//
// 保留 DUCE.ResourceType / DUCE.ResourceHandle 的书写形式以便未来直接
// 替换到 exports.cs；命名空间改为 WpfGfx.Linux.Contracts 以避免与
// System.Windows.Media.Composition 中的同名类型冲突。
...
{
    /// <summary>DUCE 静态容器（上游是 partial class，这里只为保持 DUCE.Xxx 书写形式）。</summary>
    public static class DUCE
    {
        /// <summary>MILCE 资源类型枚举。上游 wgx_core_types.cs:789，共 98 项（0x00–0x61）。</summary>
        ...
        /// <summary>
        /// ResourceHandle — 通道内资源句柄，本质是 UInt32，0 表示 Null。
        /// 上游 exports.cs:968 用 [StructLayout(LayoutKind.Explicit)] 保 4 字节布局。
        /// </summary>
        [StructLayout(LayoutKind.Explicit, Size = 4)]
        public readonly struct ResourceHandle : IEquatable<ResourceHandle>
        {
            [FieldOffset(0)] private readonly uint _handle;

            public ResourceHandle(uint handle) { _handle = handle; }
            ...
            public static explicit operator ResourceHandle(uint v) => new ResourceHandle(v);

            public uint Value => _handle;

            public bool IsNull => _handle == 0;

            public bool Equals(ResourceHandle o) => _handle == o._handle;
            public override bool Equals(object obj) => obj is ResourceHandle h && Equals(h);
            public override int GetHashCode() => (int)_handle;
            public override string ToString() => _handle == 0 ? "0x00000000(Null)" : $"0x{_handle:X8}";

            public static bool operator ==(ResourceHandle a, ResourceHandle b) => a._handle == b._handle;
            public static bool operator !=(ResourceHandle a, ResourceHandle b) => a._handle != b._handle;
        }
    }
}
