...
$exe  = "C:\u1-shaping\src\ModifierCloseOracle\bin\Debug\net10.0-windows\ModifierCloseOracle.exe"
$outA = "C:\u1-shaping\out-modifierclose"
$outB = "C:\u1-shaping\out-modifierclose2"
...
function RunOnce([string]$outDir) {
  for ($try = 1; $try -le 6; $try++) {
    ...
    $psi.WorkingDirectory = "C:\u1-shaping\src\ModifierCloseOracle"
    ...
    if ($pr.ExitCode -eq 0 -and (Test-Path (Join-Path $outDir "modifier-close-raw.json"))) {
...
$ha = Canon (Join-Path $outA "modifier-close-raw.json")
$hb = Canon (Join-Path $outB "modifier-close-raw.json")
